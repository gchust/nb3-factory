...
export async function resolveContractScope(
  ...
  for (const row of memberRows) {
    readable.add(toNumber((row as { contractId: unknown }).contractId));
  }

  const milestoneRows = await query
    .selectFrom('cdMilestones')
    .select(['id', 'contractId', 'acceptorId', 'ownerId'])
  ...
  for (const row of milestoneRows) {
    ...
    readable.add(toNumber(record.contractId));
  }
...
export function canReadContract(
  ...
  return scope.readAll || scope.readableContractIds.has(contractId);
}
...
export function canManageContract(
  ...
  return scope.manageAll || scope.managedContractIds.has(contractId);
}
