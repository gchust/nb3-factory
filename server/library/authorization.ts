...
/**
 * The business module's authorization surface: one resource, its data
 * permissions, the record-access policies the rules configure, and the
 * page resource. Everything here is registered through the application's
 * existing Authorization service — no frontend filtering and no username
...
/** The composed business resource the rules, permission sets and page all name. */
export const MATERIAL_RESOURCE_ID = 'library.materials';
/** One scope key per resource action; rules must use the same key. */
...
// A view permission is read-only. Create and edit each add their operation to a
// read base, because a repository write returns the row it wrote.
...
const materialDelete = defineDatabasePermission<MaterialRow, string>(
  (permission) =>
    // Read is included so the list route can prove, with the same Policy the
    // delete endpoint enforces, whether one row is inside the delete scope; the
    // delete write itself is still decided by the `delete` node.
    permission
      .collection<MaterialRow>(MATERIALS_COLLECTION)
      .read([...READ_FIELDS])
      .delete(),
...
/**
 * Registers everything the library needs into the running Authorization
 * service. Called once from the application provider's `start`, after the
