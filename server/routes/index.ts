...
/**
 * A tiny CRUD API for the library. Every path enforces its own security:
 * `authentication.required()` establishes the session, `authorization.middleware()`
 * resolves it into an authorization scope, and each handler authorizes the
 * matching business action before touching the Repository with its Policy.
 */
