...
| 样本当前报告 | 只有样本最终执行（同一 Run、同一 attempt）的报告才是它的当前报告，并进入 `samples[].report` 与验收 / 报告统计；执行中的样本没有当前报告，在 prepare 或被批次结束的样本为 `not-applicable`。批次在等待期后结束不等于报告齐全：缺失的仍计入 `reports.missing`、`reports-missing`，验收计入“其他”，不沿用旧 attempt 的结论 |
| 协调失败隔离 | 每个批次（及每个定时计划）单独协调：某个批次读取、建样本或派发失败时记录错误并继续处理其他批次，已导出的快照照常进入归档矩阵；协调步骤仍以失败结束，便于发现问题，下一次推进重试失败的批次且不重复创建样本 |
