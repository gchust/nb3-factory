...
QA 成功登录后按账号会话连续录制：管理员一段（`acceptance-admin.webm`）覆盖该会话
全部验收项，普通用户一段（`acceptance-normal-user.webm`）覆盖权限验收项；不再挑
1–3 个场景、不再限制单段时长。登录、注册、密码填写和退出登录不录制。实际录制使用
已有 agent-browser 0.36.0，不额外调用模型生成视频。某一步卡住时 QA 先停止录像，
恢复后另起一段；Wrapper 限制录制命令自身的等待时间，QA 返回或关闭浏览器时由
Factory 兜底保存录像。录像不可用时继续必要的截图验收，不会为录制失败开启业务修复循环。

QA 返回后工厂用 ffmpeg 对每段录像做一次内容体检，结果写入 `media-health.json`：
逐秒采样画面，整段没有可辨识的操作、或连续两分钟以上且超过一半时长没有操作的录像，
会在评论里对应标题下标注；原始文件仍保留在媒体包。体检缺失或异常不影响业务验收结果。
...
每条评论最多内嵌 40 张 PNG 和 8 段 WebM，单个文件最多 9.5 MB、总计最多 100 MB
（GitHub 对评论里的视频按免费 plan 限制 10 MB，图片同样是 10 MB）。整轮录像超过
该上限时，验收结束会用 ffmpeg 按时间重编码切成 `xxx-part-01.webm` 这样的分段，
评论里按"第 k/N 段"列出，原始整段仍保留在媒体包。其他超出预算的文件留在原始
Artifact，并在评论中说明。媒体包保留 14 天，包含截图、录像和 Markdown 清单；
不把图片或视频提交到业务代码分支，不依赖图床。
