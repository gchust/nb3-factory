...
if (mode === 'select') {
  ...
  if (!pr) {
    console.log(
      'No matching PR for this run, or the PR was updated; skipping stale media.',
    );
  } else {
    const plan = {
      ...
      prNumber: pr.number,
      headSha: pr.body.includes(`<!-- agent-head-sha: ${pr.head.sha} -->`)
        ? pr.head.sha
        : null,
  ...
  if (
    ...
    throw new Error('Publication plan mismatch');
  plan.mediaArtifactUrl =
    process.env.FACTORY_MEDIA_ARTIFACT_URL || plan.sourceArtifactUrl;
  ...
  if (!uploaded && process.env.FACTORY_MEDIA_TOKEN && plan.media.length) {
    ...
    const env = {
      ...process.env,
      GH_TOKEN: process.env.FACTORY_MEDIA_TOKEN,
      GITHUB_TOKEN: '',
      FACTORY_MEDIA_TOKEN: '',
      GH_PROMPT_DISABLED: '1',
    };
    const help = spawnSync(gh, ['pr', 'comment', '--help'], {
      env,
      encoding: 'utf8',
      timeout: 15_000,
    });
    if (help.status === 0 && help.stdout.includes('--attach')) {
      writeFileSync(
        path.join(args.output, 'comment.md'),
        renderReport(plan, true),
      );
      for (let attempt = 1; attempt <= 2 && !uploaded; attempt++) {
        ...
        console.warn(
          `::warning::Attachment publication attempt ${attempt} failed; retaining the media artifact.`,
        );
        if (attempt < 2) await sleep(2_000);
      }
      reason =
        '原生附件上传失败（请检查 Token 类型、权限或有效期）；媒体包仍可下载，业务交付不受影响。';
    } else
      reason = 'GitHub CLI 附件功能不可用；媒体包仍可下载，稍后可单独补发。';
  }
  const fallback = existing.find(
    (c) => c.user?.login === 'github-actions[bot]' && !complete(c),
  );
  if (uploaded) {
    ...
    if (!plan.media.length)
      reason = '本轮没有可发布的媒体文件，请查看原始验收 Artifact。';
    const body = renderReport(plan, false, reason);
    await api(
      fallback ? 'PATCH' : 'POST',
      fallback
        ? `/issues/comments/${fallback.id}`
  ...
  throw new Error(
    'Usage: publish-visual-report.mjs <select|prepare|publish> --run-id N ...',
  );
