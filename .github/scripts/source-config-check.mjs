...
const app = path.resolve(workspace), out = path.resolve(output);
if (!process.env.RUNNER_TEMP || !app.startsWith(`${path.resolve(process.env.RUNNER_TEMP)}${path.sep}`)) throw new Error('Only an isolated runner temporary application is accepted');
mkdirSync(out, { recursive: true });
const config = path.join(app, 'config.yml'), backup = path.join(app, 'factory-config-backup.yml');
...
try {
  ...
  checks.push({ id: 'CONFIG-03', status: 'passed', observation: 'config:set updates the effective locale; original configuration is untouched' });
  // Restore the upstream smoke's exact config/database before the login check.
  rmSync(config); renameSync(backup, config); restored = true;
  command(['config:set', 'i18n.defaultLocale=en-US', '--json']);
  ...
  try {
    ...
    for (let i = 0; i < 90; i++) {
      ...
      if (child.exitCode !== null) throw new Error('Configured application exited before login');
      try { if ((await fetch(url, { signal: AbortSignal.timeout(2000) })).ok) { ready = true; break; } } catch { /* Startup bounded below. */ }
      await delay(1000);
    }
    assert.ok(ready, 'Configured app did not become ready');
    const result = spawnSync(process.execPath, [path.join(import.meta.dirname, 'browser-smoke.mjs'),
      '--workspace', app, '--url', url, '--screenshot', path.join(out, 'login.png')], {
      env: { ...env, FACTORY_SMOKE_USERNAME: 'nocobase', FACTORY_SMOKE_PASSWORD: 'admin123' }, encoding: 'utf8', timeout: 120000 });
    assert.equal(result.status, 0, result.stderr);
    checks.push({ id: 'CONFIG-04', status: 'passed', observation: 'Configured production app starts and real browser login succeeds', evidence: 'login.png' });
  } finally {
  ...
  freezeBaseline(path.join(out, 'baseline.json'), captureBaseline(app, { sourceSha,
    controlSha: process.env.FACTORY_CONTROL_SHA ?? null, creatorVersion: process.env.FACTORY_CREATOR_VERSION ?? null }));
} catch (error) {
  checks.push({ id: 'CONFIG-FAILURE', status: 'failed', observation: error.message });
  process.exitCode = 1;
} finally {
  if (!restored && existsSync(backup)) { rmSync(config, { force: true }); renameSync(backup, config); }
  writeFileSync(path.join(out, 'configuration-checks.json'), JSON.stringify({ sourceSha, checks }, null, 2));
}
