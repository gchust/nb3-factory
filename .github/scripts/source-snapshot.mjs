...
export function verifyIntegrity(bytes, dist) {
  ...
  const strongest =
    candidates.find((x) => x.startsWith('sha512-')) ??
    candidates.find((x) => x.startsWith('sha256-'));
  if (strongest) {
    ...
    assert.match(
      dist.shasum ?? '',
      /^[a-f0-9]{40}$/u,
      'Package integrity is required',
    );
    assert.equal(
      createHash('sha1').update(bytes).digest('hex'),
      dist.shasum,
      'Package shasum mismatch',
    );
  }
}
export function validateSnapshot(value) {
  ...
  assert.ok(
    sha(value.source.sha) &&
      ...
      value.packages.length < 1000,
  );
  const names = new Set();
  for (const item of value.packages) {
    assert.ok(pkgName(item.name) && !names.has(item.name));
    names.add(item.name);
    assert.ok(
      typeof item.version === 'string' && /^[0-9][\w.+-]*$/u.test(item.version),
    );
    assert.ok(hex(item.sha256) && item.file === `packages/${item.sha256}.tgz`);
    assert.equal(item.manifest.name, item.name);
    assert.equal(item.manifest.version, item.version);
  }
  assert.ok(
    names.has('@nocobase/create-app') &&
      names.has('@nocobase/app-template-default'),
  );
  return value;
}
    ...
    for (const name of readdirSync(scope)) {
      ...
      if (versions[pkg.name])
        assert.equal(
          versions[pkg.name],
          pkg.version,
          `Multiple installed versions for ${pkg.name}`,
        );
...
export async function exportSnapshot(
  ...
  for (const [name, version] of Object.entries(
    installedScopedVersions(application, state.versions),
  ).sort(([a], [b]) => a.localeCompare(b))) {
    assert.ok(pkgName(name));
    const response = await fetcher(
      `${ORIGIN}/${encodeURIComponent(name)}/${encodeURIComponent(version)}`,
    ...
    assert.equal(tarball.origin, ORIGIN);
    const downloaded = await fetcher(tarball, {
    ...
    result.packages.push({
      name,
      version,
      origin: state.versions[name] ? 'source' : 'published-dependency',
      file,
      sha256: digest,
      manifest: metadata,
    });
...
export function loadSnapshot(directory) {
  ...
  for (const item of snapshot.packages) {
    ...
    assert.ok(
      !lstatSync(path.dirname(file)).isSymbolicLink() &&
    ...
    assert.equal(
      ...
      item.sha256,
      `Corrupt source package ${item.name}`,
    );
  }
  return snapshot;
}
export function snapshotServer(directory, snapshot = loadSnapshot(directory)) {
  ...
  const tarballs = new Map(
    snapshot.packages.map((item) => [`/snapshot/${item.sha256}.tgz`, item]),
  );
  const fingerprint = hash(JSON.stringify(snapshot));
  return createServer((request, response) => {
    if (!['GET', 'HEAD'].includes(request.method))
      return json(response, 405, { error: 'Read-only source snapshot' });
    ...
    let route;
    try {
      route = decodeURIComponent(url.pathname).slice(1);
    } catch {
      return json(response, 400, { error: 'Invalid package path' });
    }
    const entry = [...packages.values()].find(
      (p) => route === p.name || route.startsWith(`${p.name}/`),
    );
    if (entry) {
      const manifest = {
        ...entry.manifest,
        dist: {
          ...entry.manifest.dist,
          tarball: `${ORIGIN}/snapshot/${entry.sha256}.tgz`,
        },
      };
      if (route === entry.name)
        return json(response, 200, {
      ...
      if (
        [`${entry.name}/${entry.version}`, `${entry.name}/latest`].includes(
          route,
        )
      )
      ...
      if (route.startsWith(`${entry.name}/-/`) && route.endsWith('.tgz')) {
        ...
        if (route.split('/').at(-1) === expected) {
          response.writeHead(302, {
            location: `${ORIGIN}/snapshot/${entry.sha256}.tgz`,
          });
          return response.end();
        }
      }
      return json(response, 404, {
        error: 'Version absent from frozen source snapshot',
      });
    }
    if (
      route.startsWith('@nocobase/') ||
      route.startsWith('snapshot/') ||
      route.startsWith('-/')
    )
      return json(response, 404, { error: 'Not in source snapshot' });
    // Third-party dependencies still use their public registry; the lockfile retains their integrity pins.
    if (!/^(@[^/]+\/)?[^/]+(?:\/.*)?$/u.test(route) || url.search)
      return json(response, 400, { error: 'Invalid registry request' });
    response.writeHead(302, {
      location: `https://registry.npmjs.org/${url.pathname.slice(1)}`,
    });
    response.end();
  });
}
export function validateDescriptor(value, repository) {
  ...
  assert.equal(
    value.url,
    `https://github.com/${repository}/releases/download/${tag}/packages.tar.gz`,
  );
  return value;
}
export function unpackPackages(archive, output, expectedHash) {
  ...
  for (const name of listing)
    assert.match(
      name,
      /^(?:\.\/)?(?:manifest\.json|packages\/?|packages\/[a-f0-9]{64}\.tgz)$/u,
  ...
  const types = execFileSync('tar', ['-tvzf', archive], {
    ...
    .split('\n');
  assert.ok(
    types.every((line) => /^[-d]/u.test(line)),
    'Source archive links are not allowed',
  );
...
async function start(directory) {
  ...
  const child = spawn(
    process.execPath,
    [fileURLToPath(import.meta.url), 'serve', directory],
  ...
  for (let i = 0; i < 50; i++) {
    ...
    if (child.exitCode !== null)
      throw new Error('Snapshot registry could not bind to its reserved port');
    try {
      ...
      if (response.ok && (await response.json()).snapshot === fingerprint) {
        child.unref();
        writeFileSync(path.join(directory, 'server.pid'), String(child.pid));
        return snapshot;
      }
    } catch {
      /* Only startup is retried; no fallback to published NocoBase packages. */
    }
    await delay(100);
  }
  child.kill();
  throw new Error('Source registry readiness failed');
}
function configure(workspace, temporary) {
  const contents = `registry=${ORIGIN}/\n@nocobase:registry=${ORIGIN}/\nminimum-release-age=0\n`;
  const npmrc = path.join(temporary, 'source.npmrc');
  writeFileSync(npmrc, contents, { mode: 0o600 });
  writeFileSync(path.join(workspace, '.npmrc'), contents);
  // Disposable runner only. Covers the official CLI's nested/dist pnpm roots too.
  writeFileSync(path.join(os.homedir(), '.npmrc'), contents, { mode: 0o600 });
  const overrides = {
    NPM_CONFIG_USERCONFIG: npmrc,
    PNPM_CONFIG_USERCONFIG: npmrc,
    XDG_CONFIG_HOME: path.join(temporary, 'config'),
    NPM_CONFIG_REGISTRY: `${ORIGIN}/`,
    PNPM_CONFIG_REGISTRY: `${ORIGIN}/`,
    NOCOBASE_REGISTRY: `${ORIGIN}/`,
    PNPM_CONFIG_STORE_DIR: path.join(temporary, 'store'),
    PNPM_CONFIG_CACHE_DIR: path.join(temporary, 'cache'),
  };
...
async function restore(workspace, metadataFile) {
  ...
  if (!existsSync(file)) {
    const target = metadataFile
      ? readJson(metadataFile).task?.targetBranch
      : null;
    assert.ok(
      !String(target ?? '').startsWith('factory-baseline/source-'),
      'Source baseline descriptor is missing',
    );
    return; // The normal published-template path is unchanged.
  }
  const value = validateDescriptor(
    readJson(file),
    process.env.GITHUB_REPOSITORY,
  );
  assert.ok(
    process.env.RUNNER_TEMP,
    'Source snapshots require an isolated runner',
  );
  const temporary = path.join(
    process.env.RUNNER_TEMP,
    `factory-source-${value.sha256.slice(0, 16)}`,
  );
  ...
  assert.ok(
    response.ok,
    'Pinned source snapshot is unavailable; do not silently use latest',
  );
  const bytes = Buffer.from(await response.arrayBuffer());
  assert.ok(bytes.length <= 512 * 1024 * 1024);
  ...
  configure(workspace, temporary);
  console.log(
    `Restored ${snapshot.packages.length} packages from source ${value.sourceSha}`,
  );
}
async function main() {
  ...
  if (mode === 'export')
    ...
    throw new Error(
      'Usage: source-snapshot.mjs export REPO OUT SHA APP | serve DIR | start DIR [APP] | restore APP',
    );
}
if (
  process.argv[1] &&
