...
import { initialize, readState, saveState, restoreState, canStart } from '../pipeline-state.mjs';
...
const task = { issue: { number: 161 }, task: { requirements: 'Business', acceptanceCriteria: 'B01. Open\nB06. Upload file\nB16. Dashboard' } };
...
function harness(t, codes, repairs = []) {
  ...
  mkdirSync(workspace);
  for (const file of ['qa-retest.mjs', 'acceptance-criteria.mjs', 'timed-command.mjs', 'timing.mjs', 'apply-database.sh', 'template-cli.mjs'])
  ...
  write(dest, 'build-repair-prompt.mjs', "import {writeFileSync,readFileSync} from 'node:fs';if(!readFileSync(process.argv[process.argv.indexOf('--log')+1],'utf8')) throw new Error('lost repair context');writeFileSync(process.argv[process.argv.indexOf('--output')+1],'repair');");
  write(dest, 'run-agent.mjs', `import fs from 'node:fs';
...
const code=list.shift() || 0; fs.writeFileSync(process.env.REPAIRS,JSON.stringify(list));process.exit(code);`);
  ...
  write(dest, 'browser-acceptance.sh', `#!/usr/bin/env node
...
`, 0o755);
  ...
  function run(name, env = {}) {
    ...
    const result = spawnSync('bash', [path.join(scripts, 'verify-and-repair.sh'), control, workspace, prompt, metadata, artifacts, path.join(root, `${name}-state`)], {
      encoding: 'utf8', timeout: 15_000,
      env: { ...process.env, PATH: `${root}/bin:${process.env.PATH}`, TRACE: trace, CODES: codesFile, REPAIRS: repairsFile, ...env },
    });
    return { ...result, artifacts, checkpoint: path.join(artifacts, 'pipeline-state.json') };
  }
  function restore(from, next) {
    write(from.artifacts, 'agent.patch', 'same application patch');
    execFileSync(process.execPath, [path.join(scripts, 'pipeline-state.mjs'), 'seal', from.checkpoint, path.join(from.artifacts, 'agent.patch')]);
    restoreState(from.artifacts, path.join(root, next), task);
  }
  return { root, run, restore, events: () => readFileSync(trace, 'utf8').trim().split('\n') };
}
...
test('B-coded QA follows full -> focused failure -> focused success -> fresh full', (t) => {
  ...
  assert.deepEqual(h.events().filter(e => e.includes(':')), ['full:', 'focused:B06', 'focused:B06', 'full:']);
...
test('a repair handoff restores failure context and repairs before another full QA', (t) => {
  ...
  assert.equal(JSON.parse(readFileSync(path.join(first.artifacts, 'repair-summary.json'))).handoff, true);
  h.restore(first, 'two');
  const second = h.run('two');
  assert.equal(second.status, 0, second.stderr);
  assert.equal(readState(second.checkpoint).repairAttempts, 2);
  const summary = JSON.parse(readFileSync(path.join(second.artifacts, 'repair-summary.json')));
  assert.equal(summary.repairAttempts, 1);
  assert.equal(summary.finalVerificationAttempt, 2);
  assert.deepEqual(h.events().filter(e => e.includes(':')), ['full:', 'focused:B06', 'full:']);
  assert.deepEqual(h.events().slice(0, 5), ['verify', 'full:', 'repair', 'repair', 'verify']);
});
...
test('a full QA interruption does not repeat the already completed focused repair', (t) => {
  ...
  const second = h.run('two');
  assert.equal(second.status, 0, second.stderr);
  assert.deepEqual(h.events().filter(e => e.includes(':')), ['full:', 'focused:B06', 'full:', 'full:']);
});
...
test('insufficient budget checkpoints before starting full QA', (t) => {
  const h = harness(t, [0]);
  const first = h.run('one', { FACTORY_RUN_DEADLINE_EPOCH_SECONDS: String(Math.floor(Date.now() / 1000) + 200) });
  assert.equal(first.status, 75, first.stderr);
  assert.equal(readState(first.checkpoint).phase, 'qa-full');
...
test('checkpoint bindings reject a different input or different patch', (t) => {
  ...
  assert.equal(restoreState(source, path.join(root, 'dest'), task).phase, 'implementation');
  assert.throws(() => restoreState(source, path.join(root, 'other'), { ...task, task: { ...task.task, requirements: 'changed' } }), /input/u);
...
test('legacy checkpoints rebuild and QA; changed control planes reject restoration', (t) => {
  ...
  t.after(() => { if (old === undefined) delete process.env.FACTORY_CONTROL_SHA; else process.env.FACTORY_CONTROL_SHA = old; });
  ...
  state.phase = 'qa-full'; state.pendingCriteria = ['B06'];
  write(source, 'agent.patch', 'patch');
  state.patchHash = createHash('sha256').update('patch').digest('hex');
...
test('phase start estimates never exceed a fresh five-hour runner budget', () => {
  assert.equal(canStart({ fullQaSeconds: 100000 }, 'qa-full', 20000, 2200), true);
  assert.equal(canStart({ fullQaSeconds: 4000 }, 'qa-full', 10000, 9900), false);
