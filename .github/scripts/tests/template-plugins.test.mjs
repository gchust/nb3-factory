...
function inspection(name = packageName) {
  ...
  return {
    ...
    status: 'success',
    result: {
      plugin: { packageName: name, installed: true, exports: { client: true, serverPlugin: true, cli: false } },
      dependency: { field: 'dependencies', range: '^1.2.3-beta.4' },
      registration: { enabled: true },
      composition: { client: { registered: true }, server: { registered: true }, cli: { registered: false } },
      skills: { checked: true, source: [skill], synchronized: [skill], missing: [], stale: [], contentMatches: true },
      consistent: true,
      issues: [],
    },
  };
...
for (const [name, change] of [
  ['uninstalled package', (r) => { r.plugin.installed = false; }],
  ['unexpected package', (r) => { r.plugin.packageName = 'other'; }],
  ['development-only dependency', (r) => { r.dependency.field = 'devDependencies'; }],
  ['empty dependency range', (r) => { r.dependency.range = ''; }],
  ['disabled registration', (r) => { r.registration.enabled = false; }],
  ['missing client registration despite consistent=true', (r) => { r.composition.client.registered = false; }],
  ['missing server registration despite consistent=true', (r) => { r.composition.server.registered = false; }],
  ['missing client export', (r) => { r.plugin.exports.client = false; }],
  ['missing server export', (r) => { r.plugin.exports.serverPlugin = false; }],
  ['unregistered exported CLI', (r) => { r.plugin.exports.cli = true; }],
  ['unknown CLI export state', (r) => { delete r.plugin.exports.cli; }],
  ['inconsistent inspection', (r) => { r.consistent = false; }],
  ['reported warning', (r) => { r.issues.push({ severity: 'warning', code: 'SKILLS_OUT_OF_DATE' }); }],
  ['missing package-owned Skill', (r) => { r.skills.source = []; r.skills.synchronized = []; }],
  ['missing synchronized Skill', (r) => { r.skills.missing = ['missing']; }],
  ['stale Skill', (r) => { r.skills.stale = ['old']; }],
  ['changed Skill content', (r) => { r.skills.contentMatches = false; }],
  ['unchecked Skills', (r) => { r.skills.checked = false; }],
]) {
...
test('inspection fails closed on an unknown schema, failure or incomplete envelope', () => {
  for (const response of [null, {}, { ...inspection(), schemaVersion: 2 }, { ...inspection(), ok: false }, { ...inspection(), status: 'partial-success' }, { ...inspection(), result: {} }]) {
    assert.throws(() => validateInspection(response, packageName));
  }
});
...
function fixture(t, overrides = {}) {
  ...
  const state = {
    ...
    inspections: Object.fromEntries(templatePlugins.map((name) => [name, inspection(name)])),
...
test('refresh batches latest installs, registers, syncs, inspects, and records installed versions', (t) => {
  ...
  assert.deepEqual(calls[0].args, ['add', ...templatePlugins.map((name) => `${name}@latest`)]);
  for (const [index, name] of templatePlugins.entries()) {
    assert.deepEqual(calls[index + 1].args, ['--silent', 'plugin:register', name, '--no-install', '--no-skills', '--json']);
    assert.deepEqual(calls[index + 5].args, ['--silent', 'plugin:inspect', name, '--json']);
  }
  assert.deepEqual(calls[4].args, ['skills:sync']);
  assert.ok(calls.every(({ cwd }) => cwd === f.app));
  const metadata = json(path.join(f.app, 'factory-template.json'));
  assert.deepEqual({ ...metadata, plugins: undefined }, { ...f.metadata, plugins: undefined });
  assert.deepEqual(metadata.plugins, templatePlugins.map((name) => ({ packageName: name, version: '1.2.3-beta.4', registered: true, skillsSynchronized: true })));
  assert.deepEqual(json(path.join(f.diagnostics, 'template-plugins/inventory.json')), metadata.plugins);
  assert.equal(json(path.join(f.app, 'package.json')).dependencies.existing, '^1.0.0');
  assert.match(readFileSync(f.summary, 'utf8'), /verification still run before publication/);
});
...
for (const [name, overrides, count] of [
  ...
  ['partial registration with exit zero', { registerStatus: 'partial-success' }, 2],
  ...
  test(`${name} stops before stamping a successful inventory`, (t) => {
    ...
    assert.equal(existsSync(path.join(f.diagnostics, 'template-plugins/inventory.json')), false);
    assert.equal(existsSync(f.summary), false);
    if (overrides.malformed) {
      const suffix = overrides.malformed.split(':')[1];
      assert.equal(readFileSync(path.join(f.diagnostics, `template-plugins/app-plugin-ai-employee.${suffix}.json`), 'utf8'), 'not-json\n');
    }
  });
}
...
test('workflow retains default-template generation and gates verification on plugin installation', () => {
  ...
  const install = workflow.indexOf('node control/.github/scripts/template-plugins.mjs');
  assert.match(workflow, /--no-install --template=default --template-tag=latest/);
  assert.ok(install > workflow.indexOf('pnpm install --no-frozen-lockfile'));
  assert.ok(install < workflow.indexOf('Verify factory, application, clean database, and browser login'));
  assert.doesNotMatch(workflow, /app-template-pro-default|continue-on-error/);
  assert.match(workflow, /client\/plugins\.ts server\/plugins\.ts cli\/plugins\.ts/);
});
