...
if(args[0]==='start'){if(process.env.APP_CONFIG_FILE||process.env.AUTH_SECRET) process.exit(8); console.error('Run pnpm config:init');process.exit(1);}
if(args[0]==='config:init'){if(!args.includes('sqlite'))process.exit(7); if(!fs.existsSync('config.yml'))fs.writeFileSync('config.yml','auth: {secret: configured}'); console.log(JSON.stringify({ok:true}));}
else if(args[0]==='config:check')console.log(JSON.stringify({ok:true}));
else {console.error('intentional fixture stop before login');process.exit(6);}
`, { mode: 0o755 });
  const result = spawnSync(process.execPath, [path.resolve(import.meta.dirname,'../source-config-check.mjs'),app,out,'a'.repeat(40)],{
    encoding:'utf8', timeout:10000, env:{...process.env,RUNNER_TEMP:root,PATH:`${bin}:${process.env.PATH}`,APP_CONFIG_FILE:'/invalid/inherited',AUTH_SECRET:'inherited-test',CALL_LOG:path.join(root,'calls.jsonl')}
  });
  ...
  assert.equal(calls[0].override,null);assert.equal(calls[0].auth,null);
