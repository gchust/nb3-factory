...
import { deliverBundle, deliveryConfig, DeliveryConfigError, planDeliveries, sendDeliveries, targetIdOf } from '../evaluation-delivery.mjs';
...
test('planning verifies the original artifact bytes; expired or altered sources become source-expired without rebuilding', async t => {
  const receiver = await startReceiver(t);
  ...
  assert.equal(plan.items[0].source, 'source-expired');
