...
const metadata = (id = 101, extra = {}) => ({
  repository: 'owner/factory',
  issue: { number: 7 },
  controlSha: 'a'.repeat(40),
  run: { id, attempt: 1 },
  task: {
    requirements: 'Manage files',
    acceptanceCriteria: 'A cannot read B files',
...
function fixture(t, value = metadata()) {
  const root = realpathSync(
    mkdtempSync(path.join(os.tmpdir(), 'review-history-')),
  );
  t.after(() => rmSync(root, { recursive: true, force: true }));
  put(root, 'task-metadata.json', value);
  return root;
}
function invocation(root, log = 'agent-implement.jsonl', options = {}) {
  put(
    root,
    log,
    `${JSON.stringify({ type: 'tool_result', content: 'Original failed attempt' })}\n`,
  );
  put(
    root,
    `${log}.prompt.md`,
    'Original user requirement, not a generated summary.\n',
  );
  put(root, `${log}.invocation.json`, {
    version: 1,
    invoked: true,
    status: 'finished',
    ...options,
...
function capture(root, redact) {
  ...
  return {
    ...
    output,
    index: JSON.parse(
      readFileSync(path.join(output, result.input.path), 'utf8'),
    ),
  };
}
function content(result, source) {
  return (
    result.index.files
      ...
      ?.chunks.map((chunk) =>
        readFileSync(path.join(result.output, chunk.path), 'utf8'),
      )
      .join('') ?? ''
  );
}
...
test('captures original prompts, tools, implementation, repair and both QA scopes without a summary', (t) => {
  ...
  assert.match(
    content(result, 'agent-implement.jsonl'),
    /Original failed attempt/,
  );
  assert.match(
    content(result, 'agent-implement.jsonl.prompt.md'),
    /Original user requirement/,
  );
  assert.ok(
    result.index.files.every(
      (entry) => !/agent-review|comment-agent|config.yml/.test(entry.source),
    ),
  );
...
test('reuses structural and known-secret redaction before creating any reviewer file', (t) => {
  ...
  put(
    ...
    `${JSON.stringify({
      ...
      content: `Secret ${secret}; Factory-QA-abc-123`,
      nested: { password: 'another-secret' },
    })}\n`,
  );
  put(
    root,
    'agent-implement.jsonl.prompt.md',
    `Actual prompt ${secret}\nPASSWORD=not-public\n`,
  );
  const before = historyFingerprint(root);
  const result = capture(root, (text) => text.replaceAll(secret, '[REDACTED]'));
  const all = result.files
    .map((file) => readFileSync(path.join(result.output, file.path), 'utf8'))
    .join('\n');
  for (const value of [
    'private-',
    'must-not-leak',
  ...
  assert.equal(
    JSON.parse(content(result, 'agent-implement.jsonl')).nested.password,
    '[REDACTED]',
  );
  assert.equal(
    historyFingerprint(root),
    before,
    'capture never rewrites original logs',
  );
});
...
test('large logs are split at line boundaries, with a reconstructable redacted source range', (t) => {
  ...
  const text =
    [1, 2, 3]
      .map((n) => JSON.stringify({ n, content: 'x'.repeat(600_000) }))
      .join('\n') + '\n';
  put(root, 'agent-implement.jsonl', text);
  const result = capture(root),
    entry = result.index.files.find(
      (item) => item.source === 'agent-implement.jsonl',
    );
  assert.equal(entry.chunks.length, 3);
  assert.deepEqual(
    entry.chunks.map((chunk) => chunk.redactedLines),
    [
      [1, 1],
      [2, 2],
      [3, 3],
  ...
  assert.ok(
    entry.chunks.every(
      (chunk) =>
        Buffer.byteLength(readFileSync(path.join(result.output, chunk.path))) <=
        historyLimits.chunkBytes,
    ),
  );
});

test('oversized events and UTF-8 boundaries retain their complete redacted content', (t) => {
  const root = fixture(t);
  invocation(root);
  const text =
    JSON.stringify({
  ...
  assert.equal(result.input.coverage, 'available');
  assert.deepEqual(entry.omitted, []);
  const events = content(result, entry.source)
    .trim()
    .split('\n')
    .map((line) => JSON.parse(line));
  assert.equal(events[0].content, '中文🙂'.repeat(220000));
  assert.equal(events[0].password, '[REDACTED]');
  assert.equal(events[1].content, 'kept');
  assert.ok(entry.chunks[0].bytes > historyLimits.chunkBytes);
  assert.deepEqual(
    entry.chunks.map((chunk) => chunk.redactedLines),
    [
      [1, 1],
  ...
  assert.equal(result.index.invocations[0].errors[0].sourceLine, 1);
});

test('logs beyond both old byte budgets survive capture and Handoff without changing originals', (t) => {
  const root = fixture(t);
  invocation(root);
  invocation(root, 'agent-repair-1.jsonl');
  const line =
    JSON.stringify({ type: 'tool_result', content: 'x'.repeat(600000) }) + '\n';
  const text = line.repeat(60) + '{"content":"tail failure survives"}\n';
  for (const log of ['agent-implement.jsonl', 'agent-repair-1.jsonl'])
    put(root, log, text);
  const before = historyFingerprint(root),
    legacy = historyFingerprint(root, 1);
  assert.notEqual(before, legacy, 'new fingerprints bind large original logs');
  const result = capture(root);
  assert.equal(result.input.coverage, 'available');
  assert.ok(result.input.sourceBytes > 64 * 1024 * 1024);
  for (const log of ['agent-implement.jsonl', 'agent-repair-1.jsonl'])
    assert.equal(content(result, log), text);
  const next = fixture(t, metadata(102));
  preserveReviewHistory(root, next, metadata(102));
  assert.equal(
    readFileSync(
      path.join(next, 'review-history/run-101-attempt-1/agent-implement.jsonl'),
      'utf8',
    ),
    text,
  );
  assert.equal(historyFingerprint(root), before);
  assert.equal(historyFingerprint(root, 1), legacy);
});

test('missing sidecars stay partial while the original log remains available', (t) => {
  const root = fixture(t);
  put(root, 'agent-implement.jsonl', '{"content":"retained"}\n');
  const result = capture(root);
  assert.equal(result.input.coverage, 'partial');
  assert.ok(
    result.index.invocations[0].missing.includes(
      'agent-implement.jsonl.prompt.md',
    ),
  );
  assert.match(content(result, 'agent-implement.jsonl'), /retained/);
});

test('a setup failure records a non-invocation without requiring fabricated CLI output', (t) => {
  const root = fixture(t);
  invocation(root, 'agent-implement.jsonl', {
    invoked: false,
    status: 'failed',
  });
  rmSync(path.join(root, 'agent-implement.jsonl'));
  rmSync(path.join(root, 'agent-implement.jsonl.result.json'));
  const result = capture(root);
  assert.equal(result.input.coverage, 'available');
...
test('history fingerprint changes for original evidence but ignores the reviewer and mutable timing', (t) => {
  ...
  invocation(root);
  const before = historyFingerprint(root);
  put(root, 'agent-review.jsonl', 'new reviewer output');
  put(root, 'timings.jsonl', 'new timings');
  assert.equal(historyFingerprint(root), before);
...
test('Handoff preserves same-task original history separately and carries ancestors forward', (t) => {
  const first = fixture(t),
    second = fixture(t, metadata(102)),
    third = fixture(t, metadata(103));
  invocation(first);
  put(first, 'verify-1/browser-acceptance/report.json', { passed: false });
  ...
  assert.equal(result.input.coverage, 'available');
  assert.deepEqual(
    result.index.invocations.map((item) => item.runId).sort(),
    [101, 102, 103],
  );
  assert.match(
    content(result, 'review-history/run-101-attempt-1/agent-implement.jsonl'),
    /Original failed attempt/,
  );
  assert.equal(existsSync(path.join(third, 'agent-implement.jsonl')), false);
  assert.equal(
    existsSync(path.join(third, 'verify-1')),
    false,
    'past QA never becomes a current verdict',
  );
});
...
test('another Issue, changed business task, or different control plane is not imported', (t) => {
  for (const extra of [
    { issue: { number: 8 } },
    { task: { requirements: 'Other build' } },
    { controlSha: 'b'.repeat(40) },
  ]) {
    const first = fixture(t),
      second = fixture(t, metadata(102, extra));
    invocation(first);
    preserveReviewHistory(first, second, metadata(102, extra));
    const result = capture(second);
    assert.equal(result.input.invocations, 0);
    assert.ok(
      result.input.limitations.some((reason) => reason.includes('上一轮历史')),
    );
  }
});
...
test('tampered ancestor logs are not admitted as historical evidence', (t) => {
  ...
  put(
    second,
    'review-history/run-101-attempt-1/agent-implement.jsonl',
    'fabricated history',
  );
  const result = capture(second);
...
test('file and directory symlinks cannot pull unrelated credentials into review', (t) => {
  ...
  invocation(outside);
  symlinkSync(
    path.join(outside, 'agent-implement.jsonl'),
    path.join(root, 'agent-implement.jsonl'),
  );
...
test('invalid ancestor manifests cannot escape their frozen source', (t) => {
  ...
  manifest.files[0].path = '../../task-metadata.json';
  put(second, file, manifest);
  const result = capture(second);
  assert.equal(result.input.invocations, 0);
  assert.ok(
    result.input.limitations.some((reason) => reason.includes('清单无法验证')),
  );
});

// These are the production checkpoint / publication functions, not mock gates.
const { initialize, inputHash, restoreState, saveState } =
  await import('../pipeline-state.mjs');
const { collectReviewProcess, resolveReviewIdentity } =
...
test('validated checkpoint restoration also preserves history without restoring old QA as current QA', (t) => {
  ...
  put(first, 'verify-1/browser-acceptance/report.json', {
    passed: false,
    checks: [],
  });
...
test('invalid checkpoint rejects restoration before copying any ancestor evidence', (t) => {
  ...
  state.patchHash = '0'.repeat(64);
  saveState(path.join(first, 'pipeline-state.json'), state);
...
test('publication and supplement identity validation bind new raw history but keep legacy reports readable', (t) => {
  ...
  assert.throws(
    () => resolveReviewIdentity(root, report),
    /history fingerprint mismatch/,
  );
  assert.doesNotThrow(() => resolveReviewIdentity(root, { basis: {} }));
});

test('legacy publication fingerprints remain valid for original files above the former cap', (t) => {
  const root = fixture(t);
  invocation(root);
  const line =
    JSON.stringify({ type: 'message_update', text: 'x'.repeat(600000) }) + '\n';
  put(root, 'agent-implement.jsonl', line.repeat(57));
  const legacy = { basis: { historyHash: historyFingerprint(root, 1) } };
  assert.doesNotThrow(() => resolveReviewIdentity(root, legacy));
  const current = {
    basis: { historyVersion: 2, historyHash: historyFingerprint(root) },
  };
  assert.doesNotThrow(() => resolveReviewIdentity(root, current));
  put(root, 'agent-implement.jsonl', line.repeat(57) + 'tail');
  assert.throws(
    () => resolveReviewIdentity(root, current),
    /fingerprint mismatch/,
  );
  assert.doesNotThrow(() => resolveReviewIdentity(root, legacy));
});

test('event indexes locate explicit failure signals across CLI event shapes', (t) => {
  const root = fixture(t);
  invocation(root);
  put(
    root,
    ...
    [
      ...
      { type: 'message_update', text: 'streamed text is still preserved' },
    ]
      .map((value) => JSON.stringify(value))
      .join('\n'),
  );
  const result = capture(root),
    item = result.index.invocations[0];
  assert.deepEqual(
    item.errors.map((error) => error.sourceLine),
    [1, 2, 3, 4],
  );
  const index = item.eventIndexes
    .map((file) => readFileSync(path.join(result.output, file), 'utf8'))
    .join('');
  assert.ok(!index.includes('message_update'));
  assert.match(
    content(result, 'agent-implement.jsonl'),
    /streamed text is still preserved/,
  );
});
