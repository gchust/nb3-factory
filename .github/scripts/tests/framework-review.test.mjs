...
test('framework rubric has five framework dimensions; legacy four keep their original meanings', () => {
  ...
  assert.throws(
    () =>
      validateEvaluation(
        report.evaluation,
        report.basis.inputHash,
        undefined,
        1,
      ),
    /identity/,
  );
  const legacy = fixture('example.review.json');
...
test('v2 cannot relabel the old outputQuality field or silently omit a framework dimension', () => {
  ...
  review.modules[0].scores.outputQuality =
    review.modules[0].scores.requirementFit;
...
test('all five numeric dimensions require declared framework evidence, not just business code or QA', () => {
  for (const key of Object.keys(dimensionsFor(2))) {
    ...
    assert.throws(
      () => validate(review),
      /numeric framework score requires evidence/,
    );
  }
  const review = fixture().evaluation;
...
test('v2 progress remains explicit and missing source coverage permits null scores, never fabricated defaults', () => {
  ...
  review.progress = {
    complete: false,
    pendingModules: ['Unreviewed capability'],
  };
  ...
  Object.values(module.scores).forEach((score) => {
    score.score = null;
    score.reason = 'Framework source was not captured';
    score.evidence = [];
  });
  assert.doesNotThrow(() => validate(review));
});
...
test('v2 renders three primary scores, framework targets and support mapping; app findings stay separate', async () => {
  ...
  facts.buildReview = fixture();
  facts.buildReview.evaluation.modules[0].requirements[0].need =
    '<img src=x onerror=alert(1)>';
  const { html } = await renderHtml(facts, null, reports);
  const matrix = html
    .split('class="card table-wrap review-matrix framework-matrix"')[1]
    .split('</table>')[0];
  for (const label of ['需求满足度', '使用便利度', 'Agent 友好度'])
    assert.ok(matrix.includes(label));
  assert.doesNotMatch(matrix, /首轮 QA|最终轮 QA|Agent 产出质量|实现完整性/);
  for (const label of [
    '@nocobase/example-data',
    '0.0.0-fixture',
    '框架职责与业务边界',
    '正常组合',
    '业务首轮 QA',
  ...
  assert.ok(
    html.indexOf('业务调用位置错误（不直接扣框架分）') >
      html.indexOf('业务实现与执行环境观察 · 不计入框架得分'),
  );
...
test('legacy rendering preserves actual scores and says not to reuse them as framework fit/usability', async () => {
  const facts = fixture('example.facts.json');
  facts.buildReview = fixture('example.review.json');
...
test('new partial rubric may coexist with complete v1, without downgrading v2 on a stale replay', (t) => {
  ...
  const old = fixture('example.review.json'),
    next = fixture();
  next.state = 'partial';
  next.evaluation.progress = {
    complete: false,
    pendingModules: ['Next module'],
  };
  const save = (name, report) =>
    writeFileSync(path.join(root, name), JSON.stringify(report));
  save('build-review.json', old);
  save('build-review.supplement.json', next);
  let loaded = loadBuildReview(root);
  assert.equal(loaded.state, 'partial');
  assert.equal(loaded.basis.rubricVersion, 2);
  assert.deepEqual(loaded.legacyReview, old);
  assert.deepEqual(
    JSON.parse(readFileSync(path.join(root, 'build-review.json'))),
    old,
  );
  save('build-review.json', next);
  save('build-review.supplement.json', old);
...
test('a malformed original cannot prevent a valid new assessment from being adopted', (t) => {
  ...
  writeFileSync(
    path.join(root, 'build-review.json'),
    JSON.stringify({ state: 'completed' }),
  );
  writeFileSync(
    path.join(root, 'build-review.supplement.json'),
...
test('default example and runner use v2; only explicit legacy preview uses v1', () => {
  ...
  const prompt = readFileSync(
    new URL('../../prompts/build-review.md', import.meta.url),
    'utf8',
  );
  assert.match(
    runner,
  ...
  assert.match(runner, /version: rubricVersion,\s*inputHash/);
  for (const phrase of [
    '业务需求是测试场景',
    '正常编写业务规则',
    '不能仅凭错误次数',
    '旧 task.reviewCriteria',
  ]) {
