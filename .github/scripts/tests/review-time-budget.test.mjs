...
const step = workflow.split('      - name: Review build quality and framework feedback\n')[1]
  .split('\n      - name:')[0];
...
test('outer review timeout leaves time for the largest accepted invocation and checkpoint publication', () => {
  ...
  assert.ok(stepMinutes * 60 >= maximumSeconds + 120,
...
test('budget alignment retains one bounded invocation, non-scored lightweight mode and business gates', () => {
  ...
  assert.match(runner, /Math\.min\(requested, deadline - Math\.ceil\(Date\.now\(\) \/ 1000\) - 30\)/);
