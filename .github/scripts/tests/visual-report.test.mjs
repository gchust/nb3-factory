...
import { collectMedia, matchesTaskPR, renderReport, selectArtifact, text } from '../visual-report.mjs';
...
const png = Buffer.concat([Buffer.from('89504e470d0a1a0a', 'hex'), Buffer.alloc(1200)]);
const webm = Buffer.concat([Buffer.from('1a45dfa3', 'hex'), Buffer.alloc(1200)]);
...
function fixture(t) {
  ...
  write(artifacts, 'repair-summary.json', { verificationAttempts: 2, repairAttempts: 1 });
  write(artifacts, `${prefix}/report.json`, { passed: true, authenticated: true, checks: [{ status: 'passed', criterion: '资产列表', screenshots: ['page-assets.png'] }], failures: [] });
  write(artifacts, `${prefix}/showcase.json`, { pages: [{ title: '资产列表（管理员）', screenshot: 'page-assets.png' }, { title: '详情', screenshot: 'page-detail.png' }], videos: [{ title: '领用与归还', file: 'flow-checkout.webm' }], uncovered: [] });
...
test('only a successfully published same-repository task can publish media', () => {
  ...
  assert.equal(selectArtifact(run, [{ name: 'agent', conclusion: 'success' }], [artifact], repository), null);
  assert.equal(selectArtifact({ ...run, conclusion: 'failure' }, jobs, [artifact], repository), null);
  assert.throws(() => selectArtifact({ ...run, head_repository: { full_name: 'fork/repo' } }, jobs, [artifact], repository), /same-repository/);
  assert.throws(() => selectArtifact(run, jobs, [{ ...artifact, expired: true }], repository), /unexpired/);
});
...
test('binds reports to the publishing run and rejects changed PR commits', () => {
  ...
  assert.ok(!matchesTaskPR({ ...pr, head: { ...pr.head, sha: 'b'.repeat(40) } }, metadata, source));
...
test('collects final-round page screenshots and recordings without failed-round files or duplicate shots', t => {
  ...
  assert.deepEqual(plan.media.map(m => m.name), ['page-assets.png', 'page-detail.png', 'flow-checkout.webm']);
...
test('refuses handoff and failed final reports instead of substituting an earlier passing attempt', t => {
  const f = fixture(t);
  write(f.artifacts, 'repair-summary.json', { verificationAttempts: 2, handoff: true });
  assert.throws(() => collectMedia(f.artifacts, f.output), /completed verification/);
...
test('rejects traversal, symlinks, external URLs, and invalid file types without uploading other files', t => {
  ...
  symlinkSync(path.join(f.root, 'secret.png'), path.join(f.artifacts, prefix, 'evidence', 'linked.png'));
  write(f.artifacts, `${prefix}/evidence/invalid.png`, 'not a screenshot');
  write(f.artifacts, `${prefix}/showcase.json`, { pages: ['../../secret.png', 'linked.png', 'https://evil.example/a.png', 'invalid.png'].map(screenshot => ({ title: '@everyone <img>', screenshot })) });
  const plan = collectMedia(f.artifacts, f.output);
  assert.deepEqual(plan.media.map(m => m.name), ['page-assets.png']);
  assert.ok(plan.warnings.length >= 4);
});

test('oversized recordings are not attached; they remain in the original artifact', t => {
  const f = fixture(t);
  write(f.artifacts, `${prefix}/evidence/flow-checkout.webm`, Buffer.concat([webm, Buffer.alloc(9_500_000)]));
  const plan = collectMedia(f.artifacts, f.output);
  assert.equal(plan.media.filter(m => m.kind === 'webm').length, 0);
  assert.match(plan.warnings.join(' '), /原始 Artifact/);
  assert.ok(existsSync(path.join(f.artifacts, prefix, 'evidence/flow-checkout.webm')));
});

test('report markdown uses local attachments and escapes titles and mentions', t => {
  const f = fixture(t);
  const plan = { ...collectMedia(f.artifacts, f.output), runId, runUrl, runAttempt: 1, headSha, sourceArtifactUrl: `${runUrl}/artifacts/55` };
  const body = renderReport(plan, true);
  assert.match(body, /!\[.*\]\(\.\/page-assets.png\)/);
  assert.match(body, /\n\.\/flow-checkout.webm\n/);
  assert.doesNotMatch(text('@everyone <script> [link](bad)'), /<script>|@everyone|(?<!\\)\[link\]/);
...
async function publisherFixture(t, ghMode, pendingPolls = 0) {
  ...
  const server = http.createServer(async (request, response) => {
    ...
    if (route === '') result = { default_branch: 'develop' };
    ...
    else if (route === '/issues/19/comments' && request.method === 'GET') result = comments;
    else if (route === '/issues/19/comments' && request.method === 'POST') {
      const user = { login: request.headers.authorization === 'Bearer native-fixture' ? 'gchust' : 'github-actions[bot]' };
      result = { id: 10 + comments.length, body: body.body, user }; comments.push(result);
      ...
      if (request.method === 'DELETE') { comments.splice(index, 1); response.writeHead(204); response.end(); return; }
      comments[index].body = body.body; result = comments[index];
  ...
  chmodSync(gh, 0o755);
  const env = { ...process.env, GITHUB_REPOSITORY: repository, GITHUB_API_URL: `http://127.0.0.1:${server.address().port}`, GITHUB_TOKEN: 'builtin-fixture', FACTORY_MEDIA_TOKEN: '', FACTORY_GH_PATH: gh, TEST_GH_MODE: ghMode, GITHUB_OUTPUT: path.join(f.root, 'outputs') };
  const script = path.resolve(import.meta.dirname, '../publish-visual-report.mjs');
  const invoke = (mode, extraEnv = {}) => exec(process.execPath, [script, mode, '--run-id', String(runId), '--source', path.join(f.root, 'source.json'), '--artifacts', f.artifacts, '--output', f.output], { env: { ...env, ...extraEnv }, timeout: 20_000 });
  await invoke('select'); await invoke('prepare');
  return { ...f, calls, comments, invoke, setPR: value => { currentPR = value; } };
}
...
test('without a media secret, publishes/updates only its own artifact comment', async t => {
  ...
  await f.invoke('publish'); await f.invoke('publish');
...
test('two failed attachment attempts fall back to an artifact comment without failing delivery', async t => {
  ...
  const result = await f.invoke('publish', { FACTORY_MEDIA_TOKEN: 'native-fixture' });
...
test('media workflow isolates the token from QA/application execution and permits replay by run ID', () => {
  const workflow = readFileSync(path.resolve(import.meta.dirname, '../../workflows/publish-visual-report.yml'), 'utf8');
  assert.match(workflow, /workflow_run:/);
  assert.match(workflow, /workflow_dispatch:/);
  assert.match(workflow, /repository.default_branch/);
  assert.doesNotMatch(workflow, /pnpm |npm install|pull_request.head|workflow_run.head_sha/);
