...
export function parsePresetNumbers(value = '') {
  if (!value.trim()) return [];
  return [...new Set(value.trim().split(/[,，\s]+/).map((part) => {
    if (!/^#?[1-9]\d*$/.test(part) || !Number.isSafeInteger(Number(part.replace('#', '')))) {
      throw new Error(`无效的预设 Issue 编号：${part}。请填写 176,155 或 #176 #155。`);
    }
...
function validatePreset(issue) {
  ...
  if (issue.pull_request || !labels.includes('factory:preset') || labels.includes('factory:manual') ||
      !issue.user?.login || issue.user.type === 'Bot' || /\[bot\]$/i.test(issue.user.login)) {
    throw new Error(`#${issue.number} 必须是带 factory:preset 标签的人工案例，不能是 PR 或 factory:manual 任务。`);
  }
}
...
async function ensureLabel(client, name) {
  ...
  try {
    await client.request('POST', '/labels', {
      body: { name, color: 'bfdadc', description: 'Independent build tests from this preset; not a source preset' },
    });
  } catch (error) {
...
export async function runPresetTests({ client, selection, runId, dryRun = false, serverUrl = 'https://github.com' }) {
  const numbers = parsePresetNumbers(selection);
  ...
  // Validate the complete selection before creating any Issue or dispatching work.
  const sources = [];
  for (const number of numbers) {
    const issue = await client.getIssue(number);
    validatePreset(issue);
    sources.push(issue);
  }
    ...
    try {
      ...
      if (!issue) {
        ...
        issue = await client.request('POST', '/issues', {
          body: {
            title: `[Code Agent] 预设搭建测试 #${source.number}`,
      ...
      row.status = 'error';
      // Preserve partial results and attempt the other validated presets.
    }
  }
  return result;
}
...
export function renderSummary(result) {
  const heading = '# 预设搭建测试调度\n\n';
  if (!result.rows.length) return `${heading}未选择案例，没有创建 Issue 或调用 Agent。\n`;
...
async function main() {
  ...
  const result = await runPresetTests({
    client, selection: process.env.PRESET_ISSUES ?? '', runId: process.env.GITHUB_RUN_ID,
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  main().catch((error) => {
    ...
    if (process.env.GITHUB_STEP_SUMMARY) {
      appendFileSync(process.env.GITHUB_STEP_SUMMARY, `# 预设搭建测试调度失败\n\n${cell(error.message)}\n`);
    }
    process.exitCode = 1;
  });
}
