...
facts.buildReview = JSON.parse(await readFile(path.join(here, process.argv.includes('--legacy') ? 'example.review.json' : 'example.framework-review.json'), 'utf8'));
facts.meta.title = process.argv.includes('--legacy') ? '旧口径 v1 · 报告版式样例' : 'NocoBase3 基础框架评测 · 口径 v2 版式样例';
