...
} from './findings-classification.mjs';
    ...
    for (const finding of evaluation.findings) {
      ...
      const paths = [
        ...new Set(
          finding.evidence
            ...
            .filter(
              (item) =>
                item && ['code', 'package', 'skill'].includes(item.kind),
            )
      ...
      occurrences.push({
        ...
        evidence: finding.evidence
          .map((id) => evidence.get(id))
          .filter(Boolean),
        appVersion: facts.baseline?.templateVersion || null,
        appTemplate: facts.baseline?.template || null,
        upstream:
          check && upstreamEntry(check, finding)
            ? { check, entry: upstreamEntry(check, finding) }
            : null,
      });
    }
  }
...
export function clusterOccurrences(occurrences, classification = null) {
  const input = createClassificationInput(occurrences);
  const { groups } = projectClassification(input, classification);
  const byId = new Map(occurrences.map((item) => [occurrenceId(item), item]));
  return groups
    .map((group) => {
      const items = group.members.map((id) => byId.get(id));
      ...
      const type =
        items
          ...
          .find((value) => value !== 'unclassified') ?? 'unclassified';
      return {
        title: group.title,
        reason: group.reason,
...
const typeChip = (type) =>
  `<span class="ftype ftype-${type}" title="${escape(typeRubric[type] ?? '')}"><i aria-hidden="true"></i>${findingTypes[type]}</span>`;
const reportLink = (item) =>
  `../issues/${item.issue}/runs/${item.runId}/attempt-${item.attempt}/index.html`;
const upChip = (upstream) =>
  upstream
...
export async function renderFindingsIndex(
  reports,
  { generatedAt, classification = null } = {},
) {
  const { occurrences, skipped } = collectOccurrences(reports);
  const input = createClassificationInput(occurrences);
  const clusters = clusterOccurrences(occurrences, classification);
  const repeated = clusters.filter((cluster) => cluster.issues > 1);
  const single = clusters.filter(
    (cluster) => cluster.issues === 1 && cluster.reviewed,
  );
  const pending = clusters.filter((cluster) => !cluster.reviewed);
...
<meta name="factory-findings-input" content="${input.inputHash}"><meta name="referrer" content="no-referrer"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src 'unsafe-inline'; base-uri 'none'; form-action 'none'">
...
async function readReports(root) {
  ...
  for (const entry of await readdir(root, {
    ...
    if (
      ...
      !/[\\/]rubric-\d+$/.test(entry.parentPath ?? entry.path)
    )
      out.push(
        JSON.parse(
          await readFile(
...
if (
  ...
  if (!root || !output) {
    console.error(
      '用法: node findings-index.mjs <含 report.json 的目录> <输出.html>',
    );
    process.exitCode = 1;
  } else {
