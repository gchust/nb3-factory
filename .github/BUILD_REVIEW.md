...
**NocoBase3 的内部库、内置插件和开发指引 / Skill 是被评测对象，业务应用只是测试场景。**
模块要明确指出包名、公开 API 或指引路径，沿着“业务需要 → 框架公开能力 → 推荐用法 →
Agent 实际采用 → 运行结果 → 帮助/阻力”核对。不能用业务字段齐全、服务写得好或 QA 通过，
替代框架实现完整、API 合理或容易使用的结论。

主评分为需求满足度、开发易用性、Agent 友好度；详情保留框架设计合理性与框架实现完整性。
业务产出质量、首轮/最终 QA 和视觉观察在独立场景章节中展示，不计入框架主评分。

例如：表单崩溃并不直接降低框架评分；先核对 Hook 的公开约定、指引是否清楚且可发现，
再区分 Agent 没遵循、指南存在误导或框架确有缺陷。业务服务使用公开扩展点属于正常开发，
不是因为写了代码就认定框架能力不足。能力支持程度与是否被采用必须分别记录；未采用不等于不支持。

## 报告内容

| 部分 | 来源与含义 |
| --- | --- |
| 框架主评分 | 每个明确的库/插件/指引，分别评需求满足度、开发易用性、Agent 友好度；0–100 整数或 `null`，附理由与目标自身证据 |
| 框架详情 | 目标包/API/指引、需求适配、实际采用方式、框架设计合理性与实现完整性、覆盖限制 |
| 场景产出 | 应用产出质量放 `applicationOutcome`；只作场景观察，不放进框架 `scores` |
...
新评审 `basis.rubricVersion=2`、`evaluation.version=2`；外层产物 `version=1` 不变。历史 v1 混合口径仍能展示，但显著标注不可当作纯框架评分，不自动重命名或换算。要获得新口径结果，使用后补入口重新评审。

数字评分必须引用明确目标的 API / 实现 / 指引，单独的应用源码、QA 通过或包清单不能给框架打分。实现完整性必须有库/插件实现或测试证据，不能只用类型声明；需求满足度还需具体场景观察。校验只约束证据来源，不能证明归因推理正确。
