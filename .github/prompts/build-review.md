...
保存前逐个检查 scores 中五个字段的 evidence：框架证据不能只写在 targets 中而遗漏在分数字段；使用另一份 Skill 时，先把该指引列为本模块 target。找不到直接依据就保留 null。
...
引用前读取带行号的原文（如 `nl -ba`）；JSON/JSONL 必须指向包含实际字段和值的行，不能把漂亮打印的 JSON 当成单行文件，只引用首行 `{`、空行或结束括号。
纯分隔符摘录会被拒绝。引用范围应足以解释 observation；不要为了通过校验而加入无关文字。脚本不会替你扩大范围，也不能机械证明推断正确。
