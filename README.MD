...
| 类型     | 名称                             | 必填 | 说明                                                                                        |
| -------- | -------------------------------- | ---- | ------------------------------------------------------------------------------------------- |
| Secret   | `CODEBUDDY_AUTH_TOKEN`           | 是   | CodeBuddy 订阅的 OAuth Token，作为 `CODEBUDDY_AUTH_TOKEN` 注入，官方定位即 CI/CD 场景。     |
...
| Variable | `CODEBUDDY_INTERNET_ENVIRONMENT` | 否   | 网络环境标识（如 `internal`）；使用订阅 Token 时通常无需设置。                              |
