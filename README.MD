...
一个独立的 NocoBase 3 软件工厂实验仓库：你在 GitHub Issue Form 中写业务需求，GitHub Actions 调用 Code Agent 修改应用、自动验证，并向 Issue 指定的长期应用分支发起 Pull Request。

```mermaid
flowchart TD
    Issue["任意用户创建 Issue<br/>目标分支可选"] --> Queue["按 Issue 分组的并发队列"]
    Queue --> Branch{"目标分支存在？"}
    ...
    Publish --> PR["agent/issue-N → 目标分支"]
    PR -->|人工合并| Done["关闭 Issue 并唤醒下一任务"]
```

## 使用前配置

在仓库 `Settings → Secrets and variables → Actions` 中配置。仓库内置两个执行器：`pi`（默认）与 `codebuddy`；每个执行器只读取自己的配置命名空间，Workflow 按 `CODE_AGENT_ENGINE` **互斥注入**两套 Secret —— Pi 运行时拿不到订阅 Token，CodeBuddy 运行时拿不到 Pi 的 API Key 与 Endpoint。

### 引擎选择与通用配置

| 类型     | 名称                                    | 必填 | 说明                                                                                                      |
| -------- | --------------------------------------- | ---- | --------------------------------------------------------------------------------------------------------- |
| Variable | `CODE_AGENT_ENGINE`                     | 否   | 选择已注册的执行器：`pi`（留空默认）或 `codebuddy`。                                                      |
...
### CodeBuddy（`CODE_AGENT_ENGINE=codebuddy`）

| 类型     | 名称                             | 必填   | 说明                                                                                                                         |
| -------- | -------------------------------- | ------ | ---------------------------------------------------------------------------------------------------------------------------- |
| Secret   | `CODEBUDDY_API_KEY`              | 二选一 | 个人订阅在 CodeBuddy 平台密钥页获取的 API Key（`X-Api-Key`）；国内版必须同时设置 `CODEBUDDY_INTERNET_ENVIRONMENT=internal`。 |
| Secret   | `CODEBUDDY_AUTH_TOKEN`           | 二选一 | 企业旗舰版 OAuth 流程签发的 Bearer Token；与 API Key 同时配置时按 CLI 优先级使用 Token。                                     |
| Variable | `CODEBUDDY_MODEL`                | 是     | 模型 ID，例如 `deepseek-v4-pro`；缺失时任务直接失败，不会回退到订阅默认模型。                                                |
| Variable | `CODEBUDDY_THINKING`             | 否     | 默认 `max`；可设 `minimal`/`low`/`medium`/`high`/`xhigh`/`max`，`off` 表示不传 `--effort`。                                  |
| Variable | `CODEBUDDY_VERSION`              | 否     | 默认固定为 `2.150.0`；与 Pi 的版本变量相互独立。                                                                             |
| Variable | `CODEBUDDY_BASE_URL`             | 否     | 私有部署的模型服务地址；留空使用订阅默认服务。                                                                               |
| Variable | `CODEBUDDY_INTERNET_ENVIRONMENT` | 否     | 网络环境标识；用 API Key 访问国内服务时必须设为 `internal`，用 OAuth Token 时通常无需设置。                                  |

通过自定义域名使用 `deepseek-v4-flash` 或 `deepseek-v4-pro` 时，Pi 运行器会自动启用 DeepSeek 思考格式、工具调用上下文回放兼容和对应的思考等级，无需让 API 域名包含 `deepseek.com`。

CodeBuddy 执行器把配置目录指向工厂自己的 agent 目录，并以 `--setting-sources user` 只加载工厂写入的设置；生成出来的应用仓库里若存在 `.codebuddy/settings.json`（可能包含 hooks），不会被加载，订阅 Token 也不会写入任何 settings 文件。
...
1. 使用 **Code Agent 搭建任务** Issue Form 创建任务，目标分支可选，留空使用 `issues-<Issue 编号>`，也可指定 `crm`、`feature/crm` 或既有 `apps/...`。
2. 目标分支不存在时，工厂从默认分支创建；存在时继续使用它。
3. Code Agent 只修改本地工作区。流水线会运行类型检查、测试、Lint、格式检查、构建和全新 SQLite Migration/Seed；随后独立 QA Agent 使用 `agent-browser` 登录一次性数据库中的维护者账号，并按需求准备实际角色的测试账号，逐条实际操作 Issue 的验收要求，保存结构化报告和截图。只打开首页或停留在登录页不会通过。
4. 代码检查或 Agent Browser 业务验收失败时，工厂会把最新日志与浏览器复现报告交给 Code Agent，并持续执行“修复 → 代码验证 → 失败路径复测 → 完整业务验收”，不设修复次数上限。单次 Code Agent 调用默认不限时（`CODE_AGENT_INVOCATION_TIMEOUT_SECONDS=0`）；如需限制，可显式配置超时秒数。唯一的总时间上限是 GitHub 托管 Runner 单个 Job 允许的 6 小时；模型/API/浏览器基础设施错误、人工取消或超时时才标记 `agent:failed`。
...
同一目标分支只能同时有一个未合并的 Code Agent PR。同一个 Issue 的重跑和跨 Runner continuation 共用一个 `queue: max` 并发组，避免同一任务重叠执行；不同 Issue 使用不同并发组，可以同时搭建不同应用。

## 效率与交付报告

目标分支未填写时使用 `issues-<Issue 编号>`；不存在则从默认分支创建，存在则复用。工作分支仍为 `agent/issue-N`，PR 指向目标分支，旧 `apps/...` 任务无需迁移。

开发 Agent 执行针对性自测，工厂负责固定全量检查。修复循环会先自动格式化变更文件；业务失败后的下一轮先复测失败路径，通过后在全新数据库上执行完整业务 QA。局部通过不能直接发布。最终独立复验保留，修复次数不限。

实现与 QA 不共享隐藏验收指令；修复只获得失败操作和观察。历史 Agent 回复与用户需求分开，最多保留最近三条定位线索；用户已有业务约束不会因此截断。复盘仅记录真实卡点，不强制生成建议。QA 补报告保留浏览器和证据，只补缺项；验收截图可复用为展示截图。

可选 Actions Variable `FACTORY_QA_THINKING` 单独设置 QA 思考等级（由对应执行器验证）。留空继承原配置，不擅自降低默认质量；实际收益应比较同任务的完整耗时、用量和验收结果。

用量评论默认突出非缓存输入、输出和耗时，详细统计折叠。对应统计工作流的 Artifact 包含单文件 `report.html` 和 `report.json`，下载解压后直接打开，无外部依赖、不调用模型渲染。HTML 暂不部署到公共站点。QA、失败路径复测和补报告分项统计；历史混合 QA 数据保持原值。问答和媒体发布不计入搭建用量。未知用量不当作零费用，Workflow 成功不自动等于已交付。
...
调度、浏览器验收和持续修复只调用 `run-agent.mjs`。安装由 `install-agent.mjs` 完成，固定版本与执行模块在 `agent-registry.mjs` 注册。当前内置两个执行器：`pi`（默认）与 `codebuddy`。各自的 CLI 参数、配置目录、事件协议与超时收尾隔离在 `agents/pi.mjs` 与 `agents/codebuddy.mjs`，生命周期（日志脱敏、空闲看门狗、单次调用超时、Runner 预算 Handoff 退出码 `75`）由两个适配器共用 `agent-harness.mjs`。

配置按引擎隔离：Pi 只读取 `CODE_AGENT_*`（兼容旧 `PI_*`），CodeBuddy 只读取 `CODEBUDDY_*`；Workflow 依据 `CODE_AGENT_ENGINE` 只注入当前引擎的 Secret，安装步骤也使用各自的版本变量，因此给 Pi 固定的版本号不会被装到 CodeBuddy 上，反之亦然。QA 角色在两个引擎上执行同一条进程终止命令守卫：Pi 通过 `qa-process-guard.mjs` 扩展，CodeBuddy 通过 `codebuddy-qa-guard.mjs` 的 `PreToolUse` hook，两者共享 `qa-guard-rules.mjs` 的判定规则。

接入其他执行器（包括 Python 实现）时，在受信任的控制代码中新增适配器并注册，不接受 Issue 指定任意脚本路径或安装包。适配器读取属于自己的配置环境变量，接受 `--workspace`、`--prompt`、`--log`、`--agentDir`；负责非交互执行、超时终止及日志脱敏，成功退出 `0`，错误非零退出。其日志格式可由执行器决定，工厂以退出码和独立验证结果判定成功；Token 用量统计同时识别 Pi 的逐条响应事件与 CodeBuddy 的累计 `result` 事件。
...
```bash
pnpm create @nocobase/app@latest nb3-factory --no-install --template=default --template-tag=latest
```

创建器和模板继续使用 `latest`，不固定发布版本。当前创建器不再接受 `--db-dialect`；生成时使用模板默认配置，验证时由 `create-runtime-config.mjs` 显式配置独立的 SQLite 数据库。
...
刷新保留 `.github/`、`.npmrc`、`.agents/`（已有自定义指引保留，插件 Skills 按新依赖重新同步）、工厂使用说明、AGENTS 中的工厂边界，以及 `factory:test` 和浏览器测试依赖。工厂脚本的 Node.js ESLint 配置追加到新模板配置中。应用源码、NocoBase 依赖、开发指引和 Skills 都来自新模板，不从旧基线复制。按仓库所有者要求，生成的根目录 `config.yml`（包括其中的 `auth.secret`）会进入提交；每次 refresh 使用本次创建器生成的新配置，不复制旧配置。验证使用独立的外部配置和数据库，数据库、依赖目录和构建产物不会进入提交。`factory-template.json` 记录实际模板版本、刷新时间和来源控制提交。

验证显式安装 SQLite 适配器 `@nocobase/db-sqlite`，与验证配置的 dialect 一致。对 `beta.38` 发布包中的 monorepo 测试假设做定向兼容：检查生成应用的包名、使用公开包入口、校验真实路径，并按工厂策略保留 `.agents`；不跳过测试。兼容变更记录在 `factory-template.json`。
...
`1.0.0-beta.34` 默认模板附带的测试依赖上游仓库目录和固定模板名。工厂仅对该包的该版本修复测试：通过公开导出加载权限组件和迁移、使用生成应用的身份和目录、按真实路径验证 pnpm 符号链接，并补回发布时遗漏的 `.npmignore` Agent 排除规则。Vitest 仅转换相关插件的客户端文件，使原有登录状态 mock 生效。全部测试和行为断言保留；补丁先校验发布文件的 SHA-256，不匹配就终止刷新，修复记录在 `compatibilityFixes` 中。未来版本不应用这些补丁，创建器和模板仍使用 `latest`。

模板刷新自身串行，并且不再阻塞独立的搭建任务；推送时会原子地创建 `factory-backup/develop-<run-id>-<attempt>` 备份并更新 `develop`。如果有人在生成期间更新了 `develop`，Action 会拒绝覆盖。已有应用分支保持原样：测试新基线时可新建 Issue 并留空目标分支，或指定一个尚不存在的分支。仓库 Secrets、Variables 也不会被 Git 推送改动。

这个 Action 使用内置 `GITHUB_TOKEN`，无需模型 Key 或额外 Token。`develop` 的仓库规则需要允许此工作流强制推送；被规则拒绝时会报错，不会自动修改保护规则。可勾选 `dry_run` 只生成和验证，不更新远端分支。

## 任务触发权限

所有 GitHub 用户都可以通过符合模板格式的 Issue 触发搭建，不要求是仓库所有者或协作者。在有效任务 Issue 下，任何用户的 `/build` 评论均可追加搭建任务，普通评论进入只读问答；这些评论可以来自 Issue 作者之外的用户。机器人评论仍不入队，避免工厂自己的回复循环触发。

手动运行和 repository_dispatch 不再检查 Issue 作者是否为仓库所有者，但 GitHub 对这些操作本身的权限要求不变。任务仍须符合有效分支名称、必填业务字段、Issue 状态和队列规则；验收、控制文件保护与独立发布边界保持不变。模板刷新属于仓库维护操作，仍保持原有的所有者限制。

## 搭建与验收输入隔离

搭建 prompt 只接收 Issue 的业务需求、任务类型和示例数据要求，不注入验收要求或 Issue 全文链接。业务需求必须独立、完整地描述产品；QA 操作步骤、截图/录屏和评分规则只写在验收要求中。QA 仍收到完整业务需求与验收要求，验收强度不变。

自动修复保留编译和构建错误。浏览器验收失败时只传递失败项的操作及观察结果，不传递原始 QA transcript、通过项、验收条目、报告摘要或截图清单；应用启动失败则传递应用日志。各轮原始 QA 证据仍保留在独立 artifact 中，用于审计，不作为修复 prompt 输入。

这是 prompt 输入隔离，不是保密盲测：Issue 和工厂源码仍公开，Runner 内的文件也没有操作系统级隔离。Agent 被明确要求不主动读取这些内容，但不能将此要求当作访问控制。真正保密的测试需存放在非公开位置，并在没有共享文件系统的独立执行环境中运行。旧 Issue 已写进业务需求的验收文字不会自动被猜测或删除；已运行的 Agent 也无法撤回已经接收的上下文。后续任务应按上述字段边界撰写。

首次结果与修复结果应按运行及验证轮次分别查看，不能将修复后通过称为首次盲测通过；Runner handoff 后的新运行也不算全新的首次实现。
...
`pnpm config:init` writes `config.yml` from `config.example.yml`, keeping its comments and filling in `auth.secret` and `session.secret`. It is only needed once; `pnpm dev` and `pnpm start` refuse to run until it has been. `pnpm config:check` loads the configuration the way a start would, connects to any database other than SQLite, and reports what would stop the application from starting or quietly misbehave.
...
`pnpm config:init` wrote `config.yml` from `config.example.yml`, with `auth.secret` and `session.secret` filled in. It gets you running on SQLite; adjust it for your situation:

- **Database.** SQLite needs no server and works as it is. Another one means installing its driver first — `pnpm add @nocobase/db-postgres`, and `pnpm remove @nocobase/db-sqlite` if nothing else uses SQLite — then `pnpm config:init --dialect postgres --force` to rewrite the connection, and `pnpm config:set` for its settings, with `--from-env` for the password so it stays out of the shell history. Which dialects the application can run on is decided by what it depends on, not by this file: a driver it does not have cannot be introduced from here.
- **Change a value** with `pnpm config:set key=value`, which keeps the comments and refuses a section the application does not know. Values that differ per machine — `APP_SERVER_PORT`, `APP_SERVER_HOST`, `APP_PUBLIC_ORIGIN` — are better set in the environment, which is applied over this file; `config:set` says when an environment variable overrides what it wrote.
...
config.yml              Your configuration, written by pnpm config:init; gitignored
...
| Script                      | What it does                                                                                                                                                                                        |
| --------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `pnpm db:apply`             | Apply pending migrations from your `database/main/migrations/` and from every registered plugin, then pending seeds from `database/main/seeds/` and from those plugins                              |
| `pnpm db:reset`             | Destructive: drop every managed schema object, then rerun all migrations and seeds from empty; asks first, and needs `--force` in CI                                                                |
| `pnpm db:repair`            | Realign recorded migration and seed checksums with the current sources; `--dry-run` reports without writing                                                                                         |
...
Migrations and seeds also run automatically on startup while `database.connections.main.migrations.autoRun` is `true` in `config.yml`. Run `pnpm db:apply` explicitly when you want to apply a change without restarting. Both halves run only what is pending, so running it on an already-migrated database applies seeds alone.
...
`pnpm install` automatically runs `pnpm skills:sync` through the application's `postinstall` hook. Run `pnpm skills:sync` manually when install scripts were disabled or Skills need refreshing. The CLI retains `nocobase plugin skills sync` for older applications, but new templates do not provide a separate script alias.
...
`APP_BASE_PATH` is compiled into the client, so set it when building, `--build-arg APP_BASE_PATH=/crm`; it defaults to `/main`. The build runs on the builder's own architecture and cross-targets native modules, so `docker buildx build --platform linux/amd64,linux/arm64` compiles nothing under emulation.
...
The image runs as the `node` user without pnpm, so `storage/` must be writable by UID 1000, and application commands run through `node dist/cli/index.js`, as in `docker run --rm -v ./config.yml:/app/config.yml:ro my-app node dist/cli/index.js app config check`.
...
`server/config/` and `client/config/` contain editable TypeScript defaults. `config.yml` holds environment-specific overrides, written by `pnpm config:init` and yours to edit from there. It is gitignored because it carries your `auth.secret`; deploy it through your secret management rather than committing it. It is generated from `config.example.yml`, which is kept beside it as the reference for every option.
...
`HUB_API_KEY` is a Hub publishing key, created in Hub rather than in this application. Open **API Keys** in the Hub navigation (`<HUB_URL>/api-keys`); reaching that page requires `hub.app / manage-api-keys`, granted to `hub-administrator` and `hub-operator` by default. In **Create API Key**, select this application under **Applications**, or **All applications (including future apps)**, then grant the permissions the commands below need: **Upload release** for `pnpm run upload`, and both **Upload release** and **Deploy release** for `pnpm run upload --deploy` or `pnpm run deploy`.

The plaintext key is shown once at creation; while the key stays active its creator can copy it again from the same page. Bound applications and granted permissions cannot be changed afterwards, so a key created without `Deploy release` has to be deleted and recreated rather than edited. A key never exceeds its creator's current permissions, so uploads fail if the creator later loses access to the bound application. See [Publish applications with Hub](https://docs.nocobase.com/deployment/hub-publishing) for the console walkthrough.

### Upload and deploy

The Default template includes `upload` and `deploy` scripts. Run `pnpm run upload` or `pnpm run deploy` from the App root; these forward all arguments to the `app upload` and `app deploy` commands of the application's own CLI, respectively. Use `pnpm run deploy` explicitly because `pnpm deploy` is also a pnpm built-in command. Existing applications need to add these scripts if they already have the publishing CLI but predate the shortcuts.
