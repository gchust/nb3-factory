...
describe('contract delivery migrations', () => {
  it('creates every business and file collection and reverses them', async () => {
    ...
    expect(await tableExists(manager, 'cdContractFiles')).toBe(false);
    expect(await tableExists(manager, 'cdDeliverableFiles')).toBe(false);
    expect(await tableExists(manager, 'cdPaymentFiles')).toBe(false);

    await schemaMigration.down({ builder } as never);
    expect(await tableExists(manager, 'cdContracts')).toBe(false);
    expect(await tableExists(manager, 'cdMilestones')).toBe(false);
    expect(await tableExists(manager, 'cdReceivables')).toBe(false);
  });
