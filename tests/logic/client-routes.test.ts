...
describe('app client routes', () => {
  ...
  it('pins the route names page grants are stored against', () => {
    ...
    // The landing page opted out of page authorization, so it is reachable by every signed-in user. The equipment loan
    // pages do the same: they are product pages behind sign-in, not grant-managed pages.
    expect(pageAuthorizations(resolved.routes)).toEqual([
      ...
      { name: 'borrow-records', authorizedAs: null },
      { name: 'borrow-record-new', authorizedAs: null },
    ]);
  });
});
