...
describe('app client routes', () => {
  ...
  it('pins the route names page grants are stored against', () => {
    ...
    // The landing page and the customer memo pages opted out of page authorization (or sit under one that did), so
    // they are reachable by every signed-in user.
    expect(pageAuthorizations(resolved.routes)).toEqual([
      { name: 'home', authorizedAs: null },
      { name: 'customer-memos', authorizedAs: null },
      { name: 'customer-memo-new', authorizedAs: null },
      { name: 'customer-memo-detail', authorizedAs: null },
      { name: 'customer-memo-edit', authorizedAs: null },
    ]);
  });
});
