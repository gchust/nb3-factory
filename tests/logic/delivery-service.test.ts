...
describe('delivery task maintenance', () => {
  it('lets an implementation consultant maintain a task and refuses an outsider', async () => {
    const member = await contextFor(MEMBER);
    const updated = await service.updateDeliverable(member, 1, {
      status: 'in_progress',
    });
    expect(updated.status).toBe('in_progress');

    await expect(
      service.updateDeliverable(member, 1, { status: 'not-a-status' }),
    ).rejects.toMatchObject({ code: 'INVALID_STATUS' });

    const outsider = await contextFor(OUTSIDER);
    await expect(
      service.updateDeliverable(outsider, 1, { status: 'done' }),
    ).rejects.toMatchObject({ code: 'FORBIDDEN' });
  });

  it('blocks acceptance until every required task is done', async () => {
    const lead = await contextFor(LEAD);
    const member = await contextFor(MEMBER);
    await service.updateDeliverable(member, 1, { status: 'done' });

    const milestone = await service.createMilestone(lead, 1, {
      name: 'Gate milestone',
      dueDate: '2026-10-01',
      amountCents: 100_000,
    });
    const milestoneId = Number(milestone.id);
    const pending = await service.createDeliverable(member, milestoneId, {
      name: 'Pending required task',
    });
    const optional = await service.createDeliverable(member, milestoneId, {
    ...
    // A fresh unlinked file, so the gate is what rejects the submission.
    ...
    await manager
      ...
      .values({
        id: fileId,
        disk: 'local',
        key: 'objects/gate.txt',
        filename: 'gate.txt',
        ext: 'txt',
        mimeType: 'text/plain',
        size: 4,
        uploadedById: MEMBER,
        uploadedByName: 'Consultant',
        createdAt: now,
        updatedAt: now,
      } as never)
      .execute();
    ...
    await expect(
      service.submitVersion(member, Number(pending.id), { fileIds: [fileId] }),
    ).rejects.toMatchObject({ code: 'TASK_NOT_DONE' });

    await service.updateDeliverable(member, Number(pending.id), {
      status: 'done',
    });
    // The project is submitted as a whole: a required task on another milestone
    // of the same project still blocks this batch.
    const blocking = await service.createDeliverable(member, 1, {
      name: 'Blocking required task',
    });
    await expect(
      service.submitVersion(member, Number(pending.id), { fileIds: [fileId] }),
    ).rejects.toMatchObject({ code: 'REQUIRED_TASKS_INCOMPLETE' });

    await service.updateDeliverable(member, Number(blocking.id), {
      status: 'done',
    });
    const batch = await service.submitVersion(member, Number(pending.id), {
      fileIds: [fileId],
...
describe('delivery issues', () => {
  it('records an issue, scopes it to the project and validates severity', async () => {
    const member = await contextFor(MEMBER);
    const issue = await service.createIssue(member, {
      projectId: 1,
      title: 'Certificate missing',
      description: 'The customer has not supplied the production certificate.',
      severity: 'high',
    });
    expect(issue.status).toBe('open');
    expect(issue.projectNo).toBe('CT-1');
    expect(issue.canManage).toBe(true);

    await expect(
      service.createIssue(member, {
        projectId: 1,
        title: 'Bad severity',
        severity: 'urgent',
      }),
    ...
    const page = await service.listIssues(member, {});
    expect(page.items.some((row) => Number(row.id) === Number(issue.id))).toBe(
      true,
    );

    const resolved = await service.updateIssue(member, Number(issue.id), {
      status: 'resolved',
      resolution: 'Certificate received from the customer.',
    });
    expect(resolved.status).toBe('resolved');
    expect(resolved.resolution).toContain('received');

    const outsider = await contextFor(OUTSIDER);
    await expect(
      service.createIssue(outsider, { projectId: 1, title: 'Not mine' }),
    ).rejects.toMatchObject({ code: 'FORBIDDEN' });
  });

  it('reports open issues on the dashboard', async () => {
    const lead = await contextFor(LEAD);
    await service.createIssue(lead, {
      projectId: 2,
      title: 'Open dashboard issue',
      severity: 'critical',
    });
    const dashboard = await service.dashboard(lead);
    expect(dashboard.cards.openIssues).toBeGreaterThanOrEqual(1);
    expect(dashboard.recentIssues.length).toBeGreaterThanOrEqual(1);
  });
});
