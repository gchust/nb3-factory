...
const copyPackageManagerConfig = () => {
  const source = path.join(rootDir, '.npmrc');
  if (!fs.existsSync(source)) {
    return;
  }

  fs.copyFileSync(source, path.join(distDir, '.npmrc'));
};
...
// Run through Node's loader instead of the tsx CLI. The CLI opens a local IPC
// socket, which is unnecessary for this one-shot build and is unavailable in
// some restricted CI sandboxes.
run('Build workflow artifacts', 'node', [
  '--import',
  'tsx',
