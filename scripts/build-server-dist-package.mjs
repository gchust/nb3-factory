...
// Runtime dependencies the application pins to an exact version. A plugin that uses one of these
// declares a range (e.g. `@nocobase/app-plugin-authentication` declares `better-auth: ^1.7.1`)
// that a fresh `pnpm install --no-lockfile` in `dist/` would resolve to a newer release whose
// schema no longer matches the plugin's migration (better-auth 1.7.3 dropped the `issuer` column
// the plugin's migration creates as NOT NULL). Pinning here — as a direct dependency and as a
// pnpm override in the generated workspace — keeps the deployed tree on the exact version the
// application was built and verified with.
const pinnedRuntimeDependencies = ['better-auth'];
...
const writeDistWorkspace = () => {
  ...
  if (Object.keys(pinnedOverrides).length > 0) {
    lines.push(
      '',
      '# Runtime dependencies pinned to an exact version. A fresh `pnpm install --no-lockfile` would',
      '# otherwise resolve the range a plugin declares to a newer release whose schema no longer',
      "# matches the plugin's migration.",
      'overrides:',
      ...Object.entries(pinnedOverrides).map(
        ([packageName, version]) => `  ${packageName}: ${version}`,
      ),
    );
  }

  fs.writeFileSync(distWorkspacePath, `${lines.join('\n')}\n`);
};
...
/**
 * Adds the runtime dependencies the application pins to an exact version.
 *
 * These reach the server through a plugin that declares a range, so the scan over `dist` never sees the specifier
 * and the plugin's own dependency would resolve to whatever the registry currently has. Adding the pinned version as
 * a direct dependency (and as an override in the generated workspace) keeps the deployed tree on the exact version
 * the application was built and verified with.
 */
const addDeclaredPinnedRuntimeDependencies = () => {
  for (const packageName of pinnedRuntimeDependencies) {
    if (getDeclaredVersion(rootPackage, packageName)) {
      addExternalPackage(packageName);
    }
  }
};
...
addDeclaredPinnedRuntimeDependencies();
