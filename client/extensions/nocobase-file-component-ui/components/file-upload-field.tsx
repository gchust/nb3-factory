...
export function FileUploadField({
  ...
  const tooManyLabel =
    labels?.tooManyFiles ?? 'The maximum number of files has been reached.';
  const sizeRejectedLabel =
    labels?.sizeRejected ?? 'File exceeds the maximum size.';
  const typeRejectedLabel = labels?.typeRejected ?? 'File type is not allowed.';
  ...
  const addFiles = (files: readonly File[]): void => {
    ...
    const selected = (multiple ? files : files.slice(0, 1)).filter((file) => {
      if (maxSize !== undefined && file.size > maxSize) {
        onError?.(new Error(sizeRejectedLabel));
        return false;
      }
      if (!accepts(file, accept)) {
        onError?.(new Error(typeRejectedLabel));
        return false;
      }
      return true;
    });
