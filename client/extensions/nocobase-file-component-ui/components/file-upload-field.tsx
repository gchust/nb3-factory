...
/**
 * A local upload can finish before the browser paints the "uploading" state,
 * which makes the progress feedback invisible. Keep it on screen for at least
 * this long so the user always sees that work happened.
 */
...
export function FileUploadField(
  ...
  const upload = async (item: UploadItem): Promise<void> => {
    ...
    try {
      ...
      const remaining = MIN_UPLOAD_FEEDBACK_MS - (nowMs() - startedAt);
  ...
  const addFiles = (files: readonly File[]): void => {
    ...
    const selected = (multiple ? files : files.slice(0, 1)).filter((file) => {
      // Application addition: the published field accepted zero-byte uploads.
      if (file.size === 0) {
      ...
      >
        ...
        {items.map((item) => (
          <div key={item.key} className='w-36 rounded-md border p-2'>
            ...
            {item.status === 'uploading' ? (
              <div
                aria-label={t('files.uploading', { defaultValue: 'Uploading' })}
                aria-valuetext={t('files.uploading', {
                  defaultValue: 'Uploading',
                })}
