...
export function FileUploadField({
  ...
  // Authoritative accumulated list of committed records. It is updated
  // eagerly in the same tick we call `onChange`, so consecutive commit
  // drains (which run as microtasks, before React has re-rendered and
  // re-synced this ref from the `value` prop) build on the previous
  // emission instead of re-reading a stale prop value and clobbering
  // earlier completions. Without this, selecting several files at once
  // could drop some of them: each microtask drain re-spread the lagging
  // `value`, so a later drain replaced the records committed by an
  // earlier one.
  const committedRef = useRef<readonly FileRecord[]>(value);
  ...
  const cancelLabel = labels?.cancel ?? 'Cancel upload';
  const tooManyLabel =
    labels?.tooMany ?? 'The maximum number of files has been reached.';
  const tooLargeLabel = labels?.tooLarge ?? 'File exceeds the maximum size.';
  const typeNotAllowedLabel =
    labels?.typeNotAllowed ?? 'File type is not allowed.';
  const removeFailedLabel = labels?.removeFailed ?? 'File removal failed.';
  const uploadFailedLabel = labels?.uploadFailed ?? 'File upload failed.';
  ...
  const commitCompletedRecord = (record: FileRecord): void => {
    ...
    queueMicrotask(() => {
      ...
      const next = multiple
        ? [...committedRef.current, ...completed]
  ...
  const upload = async (item: UploadItem): Promise<void> => {
    ...
    try {
      ...
      const uploadError =
        error instanceof Error ? error : new Error(uploadFailedLabel);
      setItems((current) =>
        current.map((candidate) =>
          candidate.key === item.key
  ...
  const addFiles = (files: readonly File[]): void => {
    ...
    if (files.length + (multiple ? value.length : 0) + items.length > maximum) {
      onError?.(new Error(tooManyLabel));
      return;
    }
    const selected = (multiple ? files : files.slice(0, 1)).filter((file) => {
      if (maxSize !== undefined && file.size > maxSize) {
        onError?.(new Error(tooLargeLabel));
        return false;
      }
      if (!accepts(file, accept)) {
        onError?.(new Error(typeNotAllowedLabel));
        return false;
      }
      return true;
    });
  ...
  const removeRecord = async (record: FileRecord): Promise<void> => {
    if (removeOnDelete) {
      try {
        ...
        onError?.(
          error instanceof Error ? error : new Error(removeFailedLabel),
        );
        return;
      }
    }
    const next = committedRef.current.filter(
