...
/** A stable React key for a previewed record; timestamps may be either a Date or a string. */
function previewKey(file: FileRecord): string {
  const updated =
    file.updatedAt instanceof Date
      ? file.updatedAt.toISOString()
      : file.updatedAt;
  return `${file.id}:${updated}:${file.contentUrl ?? ''}`;
}
