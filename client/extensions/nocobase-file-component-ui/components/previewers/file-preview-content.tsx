...
export function FilePreviewContent(
  ...
  // Office Open XML documents (DOCX/XLSX/PPTX) are not previewed in this
  // application: the viewer dependency is not installed, so they fall through
  // to the download fallback the same way any other unsupported format does.
  if (kind === 'office') {
    return (
      <OfficePreview
        file={file}
  ...
  switch (kind) {
    ...
    case 'pdf':
      return url ? (
        <PdfPageViewer file={file} url={url} onDownload={onDownload} />
      ) : (
        <div role='status'>
          {t('files.loadingPreview', { defaultValue: 'Loading preview...' })}
        </div>
