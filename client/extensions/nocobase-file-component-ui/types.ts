...
export interface FileUiLabels {
  ...
  /** Shown when a selected file does not match `accept`; the fallback is English. */
  readonly typeRejected?: string;
  /** Shown when a selected file is larger than `maxSize`; the fallback is English. */
  readonly sizeRejected?: string;
  /** Shown when the selection would exceed `maxFiles`; the fallback is English. */
  readonly tooManyFiles?: string;
}
