...
Preview handles safe raster images, PDF through a fetched local blob, audio/video and text/Markdown. This application intentionally does not install the `@silurus/ooxml` viewer: DOCX/XLSX/PPTX and other unsupported formats fall back to a download action with an explanatory message instead of a blank preview. Legacy Office and OpenDocument formats retain Office Online, which requires an internet-accessible absolute URL and cannot use the App session. Active HTML/SVG/XML and unsafe URL schemes are rejected. Markdown does not execute raw HTML.
