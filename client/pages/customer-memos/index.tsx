...
export default function CustomerMemosPage(): ReactElement {
  ...
  // of the render that issued it, so two nearly simultaneous writes would overwrite each other.
  ...
  function scheduleSearch(value: string): void {
    ...
    // only from this timer, and "Clear filters" cancels it. If navigation happened, drop this write.
  ...
  function clearFilters(): void {
    ...
    // The "Clear filters" button disappears along with the filter; move focus to the search box.
    searchRef.current?.focus();
  }

  // Load the list. Store the result only in the request callbacks and derive "loading" from whether the
  // result belongs to the current request.
  ...
  // After a record is deleted, its row and the link that opened the dialog disappear once the list refreshes:
  // wait for the refresh to finish, then move focus to the search box.
    ...
    () => [
      {
        ...
        sortingFn: (a, b) =>
          collator.compare(a.original.customerName, b.original.customerName),
        header: ({ column }) => (
          <DataTableColumnHeader
            column={column}
            title={t('customerMemos.fields.customerName')}
          />
        ),
        cell: ({ row }) => (
      ...
      },
      {
        accessorKey: 'remark',
        enableHiding: false,
        header: t('customerMemos.fields.remark'),
        cell: ({ row }) =>
          row.original.remark ? (
            <span className='min-w-0 wrap-anywhere'>{row.original.remark}</span>
          ) : (
    ...
    <PageContainer>
      ...
      <CustomerMemoDeleteDialog
        ...
        onDeleted={() => {
          ...
          // Refresh the list, then move focus to the search box once it has finished.
          focusSearchAfterReloadRef.current = true;
          reload();
        }}
      />
      ...
      {/* The app shell mounts no toast host; a base-ui Toaster makes form and delete toasts visible. */}
      <Toaster />
    </PageContainer>
  );
}
