...
function CustomerMemoDetail({
  ...
  if (notFound || status === 403) {
    ...
    body = <CustomerMemoFields memo={memo} />;
  }
...
/**
 * Record actions at the bottom of the drawer. The footer renders inside the drawer, so
 * useRouteOverlay() can be called here. "Edit" is the only primary button in this view.
 */
function CustomerMemoDetailActions({
  ...
  return (
    <>
      ...
      {/* The delete confirmation uses component state. On success, first let the list refresh and arrange
          focus, then close the drawer. */}
...
function CustomerMemoFields({
  ...
  return (
    <dl className='grid grid-cols-[8rem_1fr] gap-x-4 gap-y-3 text-sm'>
      ...
      <dd className='min-w-0 wrap-anywhere'>{memo.customerName}</dd>
      <dt className='text-muted-foreground'>
