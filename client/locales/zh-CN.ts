import type { AppResource } from './en-US.js';

const zhCN: AppResource = {
  'auth.welcome': '欢迎回来',
  'auth.loginDescription': '使用用户名或邮箱和密码登录。',
  'auth.registerTitle': '创建账户',
  'auth.registerDescription': '创建账户以开始使用。',
  'auth.forgotTitle': '忘记密码',
  'auth.forgotDescription': '输入邮箱，如果账户存在，我们将发送重置链接。',
  'auth.resetTitle': '重置密码',
  'auth.resetDescription': '为账户设置新密码。',
  'auth.identifier': '用户名或邮箱',
  'auth.password': '密码',
  'auth.signIn': '登录',
  'auth.signInLink': '登录',
  'auth.signingIn': '正在登录…',
  'auth.hidePassword': '隐藏密码',
  'auth.showPassword': '显示密码',
  'auth.forgotLink': '忘记密码？',
  'auth.signUp': '注册',
  'auth.createAccount': '创建账户',
  'auth.creatingAccount': '正在创建账户…',
  'auth.name': '姓名',
  'auth.username': '用户名',
  'auth.email': '邮箱',
  'auth.confirmPassword': '确认密码',
  'auth.existingAccount': '已有账户？',
  'auth.resetting': '正在重置…',
  'auth.newPassword': '新密码',
  'auth.confirmNewPassword': '确认新密码',
  'auth.invalidResetLink': '此密码重置链接无效或已过期。',
  'auth.returnTo': '返回',
  'auth.sendResetLink': '发送重置链接',
  'auth.sending': '正在发送…',
  'auth.resetSent': '如果账户存在，重置链接已发送。',
  'auth.rememberPassword': '记得密码？',
  'auth.methods': '登录方式',
  'auth.continueWith': '或使用以下方式继续',
  'auth.about': '关于此应用',
  'auth.marketingDescription':
    '让 AI 在灵活的前端框架上构建体验，由 NocoBase 保障底层数据、权限、工作流与治理。',
  'auth.platform': 'AI 原生应用平台',
  'auth.frontendDescription': '在灵活的框架上自由构建界面。',
  'auth.frontend': 'AI 原生前端',
  'auth.foundationDescription': '可靠的数据、访问控制、工作流与治理。',
  'auth.foundation': 'NocoBase 基础能力',
  'auth.marketingFooter': '自由构建，可靠支撑。',
  'auth.marketingTitleFirst': '让 AI 自由构建。',
  'auth.marketingTitleSecond': 'NocoBase 保障',
  'auth.marketingTitleThird': '可靠运行。',
  'status.loading': '加载中',
  'status.loadingPage': '正在加载页面',
  'status.loadingSettings': '正在加载设置',
  'status.loadingDev': '正在加载开发工具',
  'status.denied': '无权访问',
  'status.pageFailed': '无法加载页面',
  'status.retry': '重试',
  'navigation.brandHome': 'NocoBase 首页',
  'navigation.brandApps': 'NocoBase 应用',
  'navigation.itTickets': 'IT 工单',
  'auth.passwordMismatch': '两次输入的密码不一致。',
  'status.deniedDescription': '你没有访问 {{label}} 的权限。',
  'status.routeFailedDescription':
    '无法加载 {{packageName}} 的路由 {{label}}。',
  shell: {
    workspace: 'AI 应用工作区',
    buildFreely: 'AI 自由构建。',
    reliability: '<brand>NocoBase</brand> 保障可靠。',
  },
  surface: {
    backToApp: '返回应用',
    loading: '正在加载{{title}}',
    navigation: '{{title}}导航',
    page: '{{title}}页面',
  },
  settings: {
    title: '设置',
    emptyTitle: '暂无可用设置',
    emptyDescription: '没有已启用的插件提供你有权访问的设置页面。',
  },
  dev: {
    componentExamples: '组件示例',
    title: '开发工具',
    emptyTitle: '暂无可用开发工具',
    emptyDescription: '没有已启用的插件提供你有权访问的开发页面。',
  },
  home: {
    title: '开始构建你的应用',
    description: '向 AI 助手描述你的需求，逐步构建页面、数据模型和业务流程。',
  },

  itTickets: {
    title: 'IT 工单',
    description: '提交 IT 请求并跟踪处理进度。',
    searchPlaceholder: '搜索工单',
    unassigned: '未指派',
    cancel: '取消',
    empty: '暂无工单。',
    emptyFiltered: '没有该状态的工单。',
    tabs: {
      all: '全部',
    },
    status: {
      pending: '待处理',
      'in-progress': '处理中',
      completed: '已完成',
    },
    category: {
      computer: '电脑',
      account: '账号',
      other: '其他',
    },
    columns: {
      title: '标题',
      category: '类别',
      submitter: '提交人',
      status: '状态',
      handler: '处理人',
      createdAt: '提交时间',
      actions: '操作',
    },
    actions: {
      new: '新建工单',
      view: '查看',
      start: '开始处理',
      complete: '完成',
      retry: '重试',
    },
    loadError: {
      title: '无法加载工单',
    },
    create: {
      title: '新建工单',
      description: '描述问题，IT 处理人员会尽快处理。',
      titleLabel: '标题',
      titlePlaceholder: '简要描述问题',
      titleRequired: '请输入标题。',
      categoryLabel: '类别',
      descriptionLabel: '问题描述',
      descriptionPlaceholder: '说明发生了什么，以及你已经尝试过什么',
      submit: '提交工单',
    },
    complete: {
      title: '完成工单',
      description: '记录解决“{{title}}”的处理结果。',
      noteLabel: '处理说明',
      notePlaceholder: '如何解决该工单？',
      noteRequired: '请填写处理说明。',
      submit: '标记为已完成',
    },
    detail: {
      reference: '工单 #{{id}}',
      descriptionLabel: '问题描述',
      noDescription: '未填写描述。',
      resolution: '处理结果',
      noResolution: '尚未解决。',
      submittedBy: '提交人',
      handler: '处理人',
      updatedAt: '最后更新',
    },
    toasts: {
      createdTitle: '工单已提交',
      startedTitle: '已开始处理',
      startedDescription: '你正在处理“{{title}}”。',
      completedTitle: '工单已完成',
      completedDescription: '“{{title}}”已解决。',
      errorTitle: '出错了',
    },
    errors: {
      unauthorized: '登录已过期，请重新登录。',
      forbidden: '你没有执行该操作的权限。',
      notFound: '该工单已不存在。',
      invalidState: '该工单当前状态无法修改。',
      invalidInput: '请检查填写的内容后重试。',
      generic: '请求失败，请重试。',
    },
    roles: {
      employee: 'IT 工单员工',
      handler: 'IT 工单处理人员',
    },
  },

  appearance: {
    title: '外观',
    mode: '颜色模式',
    preset: '主题',
    light: '浅色',
    dark: '深色',
    system: '跟随系统',
    themes: { default: '宽松', compact: '紧凑' },
  },
  app: {
    title: 'NocoBase',
  },
  actions: {
    close: '关闭',
    save: '保存',
    cancel: '取消',
    confirm: '确认',
    language: '语言',
  },
  notices: {
    serverLocaleFallback: '服务端不支持该语言，服务端内容已回落为英文。',
    languageChangeFailed: '未能完成语言切换，请重试。',
  },
  account: {
    signOutFailed: '退出登录失败，请重试。',
    openMenu: '打开账户菜单',
    fallback: '账户',
    signOut: '退出登录',
    signingOut: '正在退出…',
  },
  navigation: {
    home: '首页',
    open: '打开导航',
    close: '关闭导航',
    expand: '展开导航',
    collapse: '收起导航',
    label: '应用导航',
    breadcrumb: '面包屑',
  },
  dataTable: {
    noResults: '暂无数据。',
    sortAscending: '升序',
    sortDescending: '降序',
    hideColumn: '隐藏',
    view: '视图',
    toggleColumns: '显示列',
    selectedCount: '已选择 {{selected}} / {{total}} 行。',
    rowsPerPage: '每页行数',
    pageOf: '第 {{page}} 页，共 {{pageCount}} 页',
    firstPage: '第一页',
    previousPage: '上一页',
    nextPage: '下一页',
    lastPage: '最后一页',
  },
  datePicker: {
    placeholder: '选择日期',
    rangePlaceholder: '选择日期范围',
  },
};

export default zhCN;
