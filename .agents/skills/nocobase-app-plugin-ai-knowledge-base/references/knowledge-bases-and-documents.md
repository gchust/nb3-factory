...
The server enforces the LOCAL-only rule before accepting a document or running document/segment maintenance. READONLY and EXTERNAL document deletion, vectorization, segment content/question edits, enable/disable, deletion, and regeneration return HTTP 400 with code `LOCAL_KNOWLEDGE_BASE_REQUIRED` before any mutation or queue dispatch. An explicitly selected non-LOCAL base is rejected even when it has no documents; mixed batches are validated in full before execution. Read and retrieval operations are unaffected.

## Creation and update

`knowledgeBaseType` is immutable after creation. Updates may omit it or send the existing value. Any other supplied value, including `null`, returns HTTP 400 with code `KNOWLEDGE_BASE_TYPE_IMMUTABLE`; the original type, configuration, and documents remain unchanged. Create a separate knowledge base instead of attempting a conversion. This rule applies to all three types and is enforced by the server, not only by the absence of a UI control.
