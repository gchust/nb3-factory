...
Vector-database HTTP/client mutations use `connectProps`, not `connection`; `key` is optional on manual creation. Application configuration at `ai.aiKnowledgeBase.vectorDatabases` instead requires a unique `key` and a `connection` object, with optional `name` defaulting to `key`. Do not copy mutation fields directly into `config.yml`; see the [configuration and mutation examples](vector-databases.md#configuration-ownership).
...
Server segment options normalize to: enabled unless exactly false; `chunkSize` numeric coercion clamped 1..100000, default 6000 when the coerced value is falsy; `chunkOverlap` accepts finite numbers or non-empty numeric strings and clamps them to 0..`chunkSize-1`. An explicit overlap of 0 is preserved. Missing, blank, non-numeric, or non-finite overlap defaults to 1200 before the upper clamp.
...
Client plugin registration adds Knowledge bases (`/settings/ai/knowledge-base`) and Vector databases (`/settings/ai/vector-database`) as independent entries in AI Employee's `aiGroup`. `KnowledgeBaseSettingsPage` renders the list or its host-router `Outlet`; workspace/document/upload/retrieval/segment routes retain browser URLs and guards. `VectorDatabaseSettingsPage` renders only the vector page. Legacy tab URLs redirect through AI Employee; no cross-feature tabs or internal production memory router remain. The `/client/routes` named `settingsRoutes` export exposes the production contribution while its default retains the Dev contribution. See [client integration](client-integration.md#routes-and-settings).
