...
Confirm both AI Employee (the `aiGroup` owner) and Knowledge Base client plugins are registered and enabled, translations have loaded, and the user has the existing `ai.settings` page-access grant. Knowledge bases and Vector databases are separate sidebar entries at `/settings/ai/knowledge-base` and `/settings/ai/vector-database`. Production details use guarded host-router child routes, not a memory router. Use exported path helpers instead of guessing URLs. Do not duplicate the plugin's routes in the application; see [client integration](client-integration.md#routes-and-settings).
